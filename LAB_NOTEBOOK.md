...
Intent:
- Make child-WDL encoder pretraining use packed tensorized data directly so training does not have to walk raw trees and derive edge targets on the hot path.

Meaningful change:
- Extended the tensorized tree format with optional `edge_wdl_targets`, aligned with the existing flat edge arrays:
  - `edge_parent[r]`: parent node index for edge row `r`
  - `edge_child[r]`: child node index for edge row `r`
  - `edge_slot[r]`: canonical sibling slot for edge row `r`
  - `edge_wdl_targets[r]`: `(wdl_win, wdl_draw, wdl_loss)` target for child `edge_child[r]`
- `TreeTensorizer` now computes and carries `edge_wdl_targets` when the WDL feature columns are present.
- Packed tensorized shards now persist `edge_wdl_targets`, and `PackedTensorizedShardDataset` loads them directly.
- The child-WDL pretrainer now consumes precomputed `edge_wdl_targets` from packed tensorized batches when available, falling back to node-feature extraction only for non-packed paths.
...
- Added regression coverage for:
  - loading precomputed `edge_wdl_targets` from tensorized manifests
  - preserving those targets through batch collation
  - using the packed fast path without recomputing edge WDL targets from node features
