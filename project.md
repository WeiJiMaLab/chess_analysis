...
```mermaid
flowchart TD
    Xv["Node features x_v"] --> Enc["MLP_enc"]
    Enc --> GRU["GRU"]
    Hu["Child h_u"] --> Attn["Attention"]
    Attn -->|"m_up"| GRU
    Hpar["h_parent"] --> Mdown["MLP_down"]
    Mdown -->|"m_down"| GRU
    GRU --> Head["MC decision head"]
```
...
1.  **Initial encoding:** \(h_v^{(0)} = \mathrm{MLP}_{\mathrm{enc}}(x_v)\), where \(x_v\) contains node-specific features (Value \(V\), Policy \(P\), \(Q\)-values, and visit counts).
2.  **Information aggregation:**
    * **Bottom-Up** (\(m_{\mathrm{up}}\)): \(\mathrm{Attention}(h_v, \{h_u\}_{u \in \mathrm{children}(v)})\) aggregates subtree complexity.
    * **Top-Down** (\(m_{\mathrm{down}}\)): \(\mathrm{MLP}_{\mathrm{down}}(h_{\mathrm{parent}(v)})\) provides root-to-leaf strategic context.
3.  **Unified update:**
