...
graph TD
    ...
    subgraph "T Rounds of Message Passing (Unified Update)"
        ...
        %% Contextual inputs
        ChildStates["Children Hidden States<br/>h_u for each child"] --> Attn
        ParentState["Parent Hidden State<br/>h_parent"] --> MLP_Down

        %% Recurrent Loop
        GRU -.->|"State h_v, feedback"| Attn
    end
...
2.  **Information Aggregation:**
    * **Bottom-Up** (\(m_{\mathrm{up}}\)): \(\text{Attention}(h_v, \{h_u\}_{u \in \mathrm{children}(v)})\) aggregates subtree complexity.
    * **Top-Down** (\(m_{\mathrm{down}}\)): \(\text{MLP}_{\mathrm{down}}(h_{\mathrm{parent}(v)})\) provides root-to-leaf strategic context.
3.  **Unified Update:**

    $$h_v^{(t+1)} = \text{GRU}\left(h_v^{(t)}, [m_{v \leftarrow children}, m_{v \leftarrow parent}]\right)$$
