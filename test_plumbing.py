...
def build_sample_tree() -> SearchTree:
    ...
    root_id = tree.create_root(
        fen="root-fen",
        scalar_features={"value": 0.0, "prior": 1.0},
        metadata={"name": "root"},
    )
    tree.add_children(
        root_id,
...
class TensorizerTests(unittest.TestCase):
    def test_single_tree_tensorization(self) -> None:
        ...
        batch = TreeTensorizer(schema).tensorize_tree(tree)
    ...
    def test_forest_tensorization_preserves_boundaries(self) -> None:
        ...
        tree_b = SearchTree()
        root_id = tree_b.create_root("root-b", {"value": 0.0})
        tree_b.add_children(
            root_id,
            [
        ...
        batch = TreeTensorizer(schema).tensorize_forest([tree_a, tree_b])
    ...
    def test_tensorized_observation_collation_preserves_boundaries(self) -> None:
        ...
        tensorizer = TreeTensorizer(NodeFeatureSchema.from_ordered_features(["value"], defaults={"value": 0.0}))
        batch = collate_tensorized_observations(
            [
                tensorizer.tensorize_tree_observation(tree_a),
                tensorizer.tensorize_tree_observation(tree_b),
            ]
        )
