...
are good). This is in `lmcos_small/analysis/normative_curves.py`. It is the first evidence that the
...
> (`lmcos_small/analysis/normative_curves.py`) — the move-count effect *falls out of* a
...
| A resource-rational (per-op cost) stop **reproduces** +size and −satisfaction | **Settled (one sign)** | `lmcos_small/analysis/normative_curves.py` |
...
- **Engine** — the engine value signals (Gain / MQ / greedy gap / GSS / frac-good / OSS) on the SF1
  `n1md36` trees, computed **inline** by `lmcos_small/human/engine.py` (figures in `figures/engine/`).
...
- **Normative** — [(R-TREESEARCH)](reports/treesearch.md): the resource-rational framing and the
  ...
  `lmcos_small/analysis/normative_curves.py`.
