...
**Pinned lc0 behaviors (2026-06-14, confirmed empirically against the binary).** Building the in-process net surfaced three lc0 defaults the `build_tree.py` configs silently inherited — all now reproduced under `re_baseline=False` and confirmed to match lc0 exactly on startpos (value +0.0740, wdl 0.246/0.582/0.172; priors d2d4 0.1806 vs lc0 0.1804):

1. **Value is a 1-ply minimax, not the raw net value head.** lc0's `valuehead` search (even at `go nodes 1`) evaluates the root's children and backs up the best: `value = max over legal children of (-raw_value(child))`. Confirmed identical to lc0. This 1-ply child evaluation *is* the per-tree cost (R-U1-SPEED's "child value-eval loop"); batched in-process it's still the speedup, just ~30× more net calls than the raw head.
2. **Priors carry `PolicyTemperature` 1.359** — `P = softmax(logits / 1.359)` over legal moves (then 1e-4 text rounding). `T=1.359` reproduces lc0's reported P to ~1e-4.
3. **History is `fen_only`** (lc0's `HistoryFill` default) — lczerolens's default `LczeroBoard` encoding matches it; children are evaluated with the move pushed (1 ply of history), as lc0's valuehead search does.

`re_baseline=True` (the future clean path) uses the **raw root value head + temperature 1.0** — ~30× cheaper and a cleaner training target (no 1-ply "search-in-the-feature"); it differs from the existing trees, so adopting it requires re-running the A0 oracle-direction checks. Also note: the onnx2torch graph must be put in **`eval()` mode** or BatchNorm uses per-batch statistics, making outputs batch-dependent (caught by the pinning test).
