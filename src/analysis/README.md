# `src/analysis/` — deliberation / RT analysis
...
so it has a visible, documented home (mirrors `src/human/`'s layout).
...
`slurm/helpers/setup_env.sh` and calls `cts.data.build_tree` with
...
`halter_vs_rt.py`) need `cts`; they add `src` to `sys.path` and import
