"""
Analysis of the relationship between Number of Possible Moves and move time, segmented by game phase.
Phases (Opening, Midgame, Endgame) are pre-defined in the database based on move ply quantiles.
This script re-calculates possible moves quantiles within each phase and plots the results.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Phase-segmented possible moves analysis.")
    ...
    # 2. Segmented Quantile Calculation for n_possible_moves
    print("\nCalculating per-phase n_possible_moves quantiles...")
    query = f"""
    WITH binned_data AS (
        SELECT 
            game_phase,
            n_possible_moves,
            ln_move_time,
            ntile(20) OVER (PARTITION BY game_phase ORDER BY n_possible_moves) as qbin
        FROM {base_table}
    )
    SELECT 
        game_phase,
        qbin,
        avg(n_possible_moves) as mean_npossible,
    ...
    for i, phase in enumerate([1, 2, 3]):
        ...
        ax.errorbar(
            subset['mean_npossible'], subset['mean_y'], 
        ...
        if args.overlaid:
            ...
            ax.set_xlabel("Possible Moves", fontsize=FONT_SIZE_LABEL)
    ...
    output_path = os.path.join(figures_dir, f"phase_segmented_npossible_{suffix}.png")
