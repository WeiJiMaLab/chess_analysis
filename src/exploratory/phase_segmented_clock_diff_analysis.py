"""
Analysis of the relationship between CLOCK DIFFERENCE (Player - Opponent) and move time, segmented by game phase.
Phases (Early, Mid, End) are pre-defined in the database based on move ply quantiles.
This script re-calculates clock difference quantiles within each phase and plots the results.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Phase-segmented clock difference analysis.")
    ...
    # 2. Segmented Quantile Calculation for Clock Difference
    print("\nCalculating per-phase clock difference quantiles...")
    query = f"""
    WITH phase_data AS (
        SELECT 
            game_phase,
    ...
    binned_data AS (
        SELECT 
            *,
            ntile(20) OVER (PARTITION BY game_phase ORDER BY clock_diff) as qbin
        FROM phase_data
    )
    SELECT 
        game_phase,
        qbin,
        avg(clock_diff) as mean_clock_diff,
    ...
    for i, phase in enumerate([1, 2, 3]):
        ...
        ax.errorbar(
            subset['mean_clock_diff'], subset['mean_y'], 
        ...
        if args.overlaid:
            ax.fill_between(
                subset['mean_clock_diff'], 
                subset['mean_y'] - subset['sem_y']*2,
                subset['mean_y'] + subset['sem_y']*2,
                color=PHASE_COLORS[phase], alpha=0.1
            )
        else:
            ax.axvline(0, color='black', linestyle='--', alpha=0.5, lw=2) # Zero difference line
            ...
            ax.set_xlabel("Mean Clock Difference (Self - Opponent) [s]", fontsize=FONT_SIZE_LABEL)
            # Add labels for "Ahead" and "Behind"
            ax.text(0.95, 0.05, "Ahead in Time →", transform=ax.transAxes, ha='right', fontsize=FONT_SIZE_TICKS - 5, alpha=0.6)
            ax.text(0.05, 0.05, "← Behind in Time", transform=ax.transAxes, ha='left', fontsize=FONT_SIZE_TICKS - 5, alpha=0.6)

    if args.overlaid:
        ax.axvline(0, color='black', linestyle='--', alpha=0.5, lw=2)
        ax.set_xlabel("Clock Difference (Self - Opponent) [s]", fontsize=FONT_SIZE_LABEL)
        ax.set_ylabel("Mean log T (s)", fontsize=FONT_SIZE_LABEL)
    ...
    output_path = os.path.join(figures_dir, f"phase_segmented_clock_diff_{suffix}.png")
