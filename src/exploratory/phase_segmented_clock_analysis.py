...
This script re-calculates clock quantiles within each phase and plots the results.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Phase-segmented clock analysis.")
    ...
    if args.overlaid:
        ...
        axes = [ax] * 3 # Hack to keep the loop logic similar
    else:
    ...
    for i, phase in enumerate([1, 2, 3]):
        ...
        if args.overlaid:
            ...
            ax.set_xscale('log')
            ax.set_xlabel("Mean Remaining Clock (s) [log scale]", fontsize=FONT_SIZE_LABEL)
    ...
    output_path = os.path.join(figures_dir, f"phase_segmented_clock_{suffix}.png")
