...
1. **Select Games**: `python src/utils/load_data.py select_games` builds `selected_games` from `core.games`.
2. **Sharding**: `load_moves_shards.sbatch` partitions move extraction across multiple nodes.
3. **Merging**: `load_moves_merge.sbatch` unifies the shards into the personal database.
4. **Quality Control**: We automatically **exclude entire games** that contain any negative move times (artifacts of lag compensation or manual clock additions) to ensure the integrity of our behavioral models.
...
1. **Select Games**: `python src/utils/load_data.py select_games` (Builds `selected_games` from `core.games` using Nov-Dec 2023, 10+0, both Elo ≥ 2000).
2. **Extraction**: `bash src/slurm/script_load_moves.sh` (Pulls moves for selected games).
3. **Processing**: `python src/script_process_data.py` (Calculates basic features).
4. **Analysis**: 
   - `python src/move_time_summary.py` (SQL-binned move-time / log move-time histograms → `figures/move_time_summary/`).
   - `python src/clock_movetime.py` (Player or opponent clock vs. move time, 2×2 panel → `figures/clock_movetime/`; use `--opp` for opponent clock, `--nonzero_T` as needed; outputs `combined.png`, `combined_opp.png`, `combined_nonzero_T.png`, `combined_nonzero_T_opp.png`).
   - `python src/npossiblemoves_movetime.py` (Number of legal moves vs. raw move time, 2×2 panel → `figures/npossiblemoves_movetime/`; use `--nonzero_T` as needed; outputs `combined.png`, `combined_nonzero_T.png`).
   - `python src/ply_movetime.py` (Ply stage vs. log move time → `figures/ply_movetime/`).
