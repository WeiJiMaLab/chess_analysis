...
4. **Analysis**: 
   ...
   - `python src/clock_movetime.py` (Player clock vs. move time; use `--opp` for opponent clock).
   - `python src/npossiblemoves_movetime.py` (Number of legal moves vs. raw move time).
   - `python src/ply_movetime.py` (Ply stage vs. log move time, side-by-side view).
   - `python src/ply_premove.py` (Ply stage vs. probability of "instant moves" (T=0); side-by-side view → `figures/ply_instantmove/`).
