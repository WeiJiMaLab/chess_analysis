...
def _save_encoder_training_curves(history: list[dict], output_checkpoint: str,
    ...
    ``curves_out_dir``, if given, writes to ``<curves_out_dir>/encoder/training_curve.*`` (its own
    subfolder, sibling to the readout heads' own curves under the same eval-pipeline output dir --
    NOT flat inside ``curves_out_dir`` itself, where a same-named file from another stage could
    collide with it) instead of next to ``output_checkpoint`` in scratch.
    ...
    if curves_out_dir:
        # Own "encoder" subfolder, not flat inside curves_out_dir: the readout-head curves
        # (stats/zt/ag) live there too, and a from-scratch curves_out_dir regenerated by an
        # eval-only rerun (which never retrains the encoder) has no reason to know these
        # files exist -- keeping them siloed means eval's own writes can never collide with
        # or orphan them.
        base = Path(curves_out_dir) / "encoder" / "encoder"
    else:
