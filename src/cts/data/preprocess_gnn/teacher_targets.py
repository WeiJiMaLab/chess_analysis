...
class PretrainExample:
    ...
    Supervises per-node value backups and (optionally) per-edge WDL targets for
    the Child-WDL pretrain path.
    """
...
class RawPretrainExampleRecord:
    ...
    def to_tensorized_tree_example(self, schema) -> TensorizedTreeExample:
        """Fill ``node_features`` exactly as configured by packing."""

        expected = tree_encoder_feature_schema()
        if tuple(schema.feature_names) != tuple(expected.feature_names):
            raise ValueError(
                ...
                f"{tuple(expected.feature_names)!r}. "
...
def _prune_children(
    ...
    """Value-prune a node's children during the build (depth>=1).

    NEGAMAX: a child's value is from the child's (mover's) perspective, so the parent
    prefers the MIN-value child; the best (min) child is ALWAYS kept so a node never
    becomes a childless dead-end. Pruned children are never attached, so the freed budget
    redeploys deeper on the survivors. Modes (``param`` is the single knob):
      ``relative``: keep value <= min + param          (within ``param`` of the best; eps)
      ``absolute``: keep value <= 1 - 2*param           (mover win-prob >= ``param``)
      ``rank``    : keep the ``param`` best (lowest-value) children
    ``mode=None`` is a no-op.
    """
    kids = list(children)
    if not mode or param is None or len(kids) <= 1:
        return kids
    ...
    if mode == "relative":
        kept = [c for c, v in zip(kids, vals) if v <= best + param]
    elif mode == "absolute":
        ceil = 1.0 - 2.0 * param           # param = mover win-prob floor; lower value = better
        kept = [c for c, v in zip(kids, vals) if v <= ceil]
    elif mode == "rank":
        k = max(1, int(round(param)))
        keep_idx = set(sorted(range(len(vals)), key=lambda i: vals[i])[:k])
        kept = [c for i, c in enumerate(kids) if i in keep_idx]
    else:
        raise ValueError(f"unknown prune_mode {mode!r}")
