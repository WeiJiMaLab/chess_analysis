...
Run from repo root::
    ...
    PYTHONPATH=src python3 src/slurm/scripts/tests/compare_legacy_new_pipeline_smoke.py \\
...
# ``src/`` for ``utils`` if needed; ``scripts/`` for ``preprocess``
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "``src/slurm/scripts/tests/boundary_suite_results.json`` (and per-window JSON next to it). "
