...
#   bash src/slurm/engine_eval.sh --no-wait   # submit only; then --merge-only when the array is done
...
if [[ "${NO_WAIT}" -eq 1 ]]; then
  ...
  JID=$(
    sbatch --parsable --export=ALL,ENGINE="${ENGINE}",LIMIT="${LIMIT}" \
      "${PROJECT_DIR}/src/slurm/engine_eval_shard.sbatch"
  )
  echo "Submitted Slurm array job: ${JID} (logs: src/slurm/logs/eval_*.out)"
  echo "Not waiting (--no-wait). When the job completes, run:"
  echo "  bash src/slurm/engine_eval.sh --merge-only --engine ${ENGINE} --db ${DB} --input-dir ${INPUT_DIR}"
  exit 0
fi
...
set -e
if [[ "${_wc}" -ne 0 ]]; then
  echo "sbatch --wait failed (exit ${_wc}); not running merge. Check src/slurm/logs/ and re-run with --merge-only when parquets are ready." >&2
  exit "${_wc}"
fi
echo "Array job ${JID} finished at $(date). Logs: src/slurm/logs/eval_*.out"
