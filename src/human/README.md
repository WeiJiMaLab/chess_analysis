# Chess Thinking Dynamics — `src/human/` handbook

**Audience:** coding agents, research assistants, and human contributors working in `chess_analysis/src/human/`.
...
All paths are relative to the **`chess_analysis/`** repo root (parent of `src/human/`).

| Task | Command |
| :--- | :--- |
| **Activate env** | `source .venv/bin/activate` |
| **Moves ETL (games → shards → merge → `process_moves`)** | `bash slurm/human/preprocess.sh` (or `preprocess.py get_games` / `shard` / `merge` / `process_moves` separately) |
| **Regenerate standard figures** | `bash slurm/human/analysis.sh` |
| **log(MT) histogram + normal QQ** | `python src/human/move_time_summary.py` |
| **Move-time dashboards** (clock, branching, own non-pawn material, ply) | `python src/human/movetime_analysis.py` (optional: `--only clock legal_moves own_material ply`) |
| **Ply vs instant-move probability** | `python src/human/ply_premove.py` |
| **Tree-derived GSS / VOC / Action Gap / MQ vs RT** (lc0-tree subset) | `sbatch slurm/human/tree_values.slurm` (`tree_values_analysis.py`; not part of the full-dataset pipeline) |
| **Slidev deck (LMCOS overview)** | `cd src/human/presentations/lmcos-overview && npm install && npm run dev` (symlink `public/figures` per that README) |

The **full-dataset** plots (`move_time_summary`, `movetime_analysis`, `ply_premove`) are wired from **`bash slurm/human/analysis.sh`**. The **generated values** (GSS / VOC / Action Gap / **MQ**, derived from the lc0 search trees) are computed on the **subset of positions that have a tree** via `tree_values_analysis.py` and run separately on the cluster. **MQ moved from FULL to SUBSET**: it is now the Lc0 definition — the post-search root-value loss of the human's played move, `final_Q(played) − final_Q(best) ≤ 0` — not the former Stockfish `pos_with_engine_eval.mq` (`e_win_taken − e_win_best`), which has been retired.
...
└── src/human/
    ├── README.md                 # This file
    ├── exploratory/              # Ad hoc analyses (heatmaps, smoke tests, quantify_early_ply); PYTHONPATH=src/human
...
#   slurm/human/{preprocess.sh, analysis.sh, tree_values.slurm, logs/}
```
...
| Location | Put here |
| :--- | :--- |
| **`src/human/preprocess/`** | New **ETL / engine / join** entry points. Run with `PYTHONPATH=src/human` so `import utils` resolves (no sys.path shim); the batch launcher goes in `slurm/human/`. |
| **`src/human/`** (top-level `.py`) | New **dashboards, reports, thin CLIs** that read `personal.db` (see `utils/selected_db.py`) and write figures. |
| **`src/human/utils/`** | **Reusable** plotting, SQL aggregation patterns, `Analyzer`/`Variable`—**not** one-shot pipeline drivers. |
| **`src/human/exploratory/`** | Experiments and one-off plots; follow existing `sys.path` patterns. |
| **`src/human/presentations/`** | Slidev decks (`lmcos-overview/`) and presentation assets only—not Python pipeline code. |
...
**Orchestration:** `bash slurm/human/preprocess.sh`:

- Clears **`staging_dir`** and prior **`ld-moves_*`** logs in **`slurm/human/logs/`** (does not remove **`eval_*`**).
...
`nohup bash slurm/human/preprocess.sh >> slurm/human/logs/preprocess_driver.log 2>&1 &`
...
1. `bash slurm/human/preprocess.sh` (or equivalent `preprocess.py` steps) → `processed_moves[_nonzero]`.
2. **Figures:** `bash slurm/human/analysis.sh` or individual `src/human/*.py` tools in §1.
...
- **Paths:** `os.path.join` + anchor to `__file__`; for the `preprocess/` CLIs rely on `PYTHONPATH=src/human` (set by the launcher) rather than a sys.path shim.
