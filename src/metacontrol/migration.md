...
5. [x] Implement `src/metacontrol/scripts/sample.py` for root FEN extraction from Lichess DB (`ChessSampler`), plus fast `compose_full_fen` unit tests (`pytest -m "not integration"`).
