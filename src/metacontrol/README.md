...
Run `python -m metacontrol.scripts.generate_and_profile` from `src/` with `PYTHONPATH` set to `.../chess_analysis/src`. **Defaults** in that script: LC0, `nodes=64` per expansion, `max_nodes=64` PUCT budget, `max_depth=10`, `c_puct=1.25`, example CSV and output paths under `/scratch/gpfs/GRIFFITHS/...`.
