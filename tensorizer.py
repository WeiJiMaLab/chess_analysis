...
class TreeBatch:
    ...
    num_nodes: int
    num_edges: int
    edge_wdl_targets: Optional[torch.Tensor] = None
...
class TreeTensorizer:
    ...
    def edge_wdl_target_tensor(
        ...
        for node in tree.iter_nodes():
            for child_id, _slot in self._sorted_child_ids_with_slots(tree, node.node_id):
                ...
                target = torch.tensor(edge_wdl_targets[edge_key], dtype=torch.float32, device=self.device)
                if target.shape != (3,):
    ...
    def tensorize_tree_observation(
        ...
        *,
        ...
        for node in tree.iter_nodes():
            ...
            for child_id, slot in self._sorted_child_ids_with_slots(tree, node.node_id):
    ...
    def tensorize_forest(self, trees: Sequence[SearchTree], *, validate: bool = True) -> TreeBatch:
        ...
        for batch_idx, tree in enumerate(trees):
            ...
            for node in tree.iter_nodes():
                ...
                child_id_slots = self._sorted_child_ids_with_slots(tree, node.node_id)
...
def tensorize_tree_with_targets(
    ...
    tensorizer = TreeTensorizer(schema=schema, device=device)
    ...
    return TensorizedTreeExample(
        ...
        edge_wdl_targets=(
            tensorizer.edge_wdl_target_tensor(tree, edge_wdl_targets)
