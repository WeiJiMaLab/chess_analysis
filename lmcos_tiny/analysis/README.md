# `lmcos_tiny/analysis/` — deliberation / RT analysis
...
so it has a visible, documented home (mirrors `human_analytics/`'s layout).
...
analysis/
  ...
  logs/                     # slurm .out/.err land here
```

## Compute → figure flow

1. **`compute_voc_signals.py`** (sbatch `slurm/voc_signals.slurm`) — reads the filtered SF
...
2. **`compute_voc_tau_sweep.py`** (sbatch `slurm/voc_tau_sweep.slurm`) — softmax-VOC over a
...
`lmcos_tiny/pipeline/helpers/setup_env.sh` and calls `cts.data.build_tree` with
...
`halter_vs_rt.py`) need `cts`; they add `lmcos_tiny/src` to `sys.path` and import
...
| Figures | `/home/hl4291/chess_analysis/figures/lmcos_tiny/` |
