#!/bin/bash
# Submit the entire lmcos_tiny pipeline (packing -> pretrain -> materialize -> readout)
...
for ELO in "${RUNGS[@]}"; do
  ...
  FILTER_ID=$(sbatch --parsable --export=ALL,ELO=${ELO} --array=0-$((NFILTER-1)) pipeline/1b_filter_trees.slurm)
  ...
  PACK_ID=$(sbatch --parsable --export=ALL,ELO=${ELO} --dependency=afterok:${FILTER_ID} pipeline/1c_pack_trees.slurm)
  echo "  1c_pack_trees: ${PACK_ID}"

  # 2a. train_encoder (GPU)
  ENC_ID=$(sbatch --parsable --export=ALL,ELO=${ELO} --dependency=afterok:${PACK_ID} pipeline/2a_train_encoder.slurm)
  echo "  2a_train_encoder: ${ENC_ID}"

  # 2b. pack_root (materialize worker array, GPU)
  REPS_ID=$(sbatch --parsable --export=ALL,ELO=${ELO},NWORKERS=${NWORKERS} --array=0-$((NWORKERS-1)) --dependency=afterok:${ENC_ID} pipeline/2b_pack_root.slurm)
