...
**Orchestration:** `bash lmcos_small/slurm/human/preprocess.sh`:

- Clears **`staging_dir`** and prior **`ld-moves_*`** logs in **`lmcos_small/slurm/human/logs/`** (does not remove **`eval_*`**).
...
`nohup bash lmcos_small/slurm/human/preprocess.sh >> lmcos_small/slurm/human/logs/preprocess_driver.log 2>&1 &`
