# `lmcos_small/analysis/` — deliberation / RT analysis
...
so it has a visible, documented home (mirrors `lmcos_small/human/`'s layout).
...
`lmcos_small/pipeline/helpers/setup_env.sh` and calls `cts.data.build_tree` with
