...
| Deck | Entry | Topic |
| --- | --- | --- |
| `main` | `src/main.md` | Learned meta-control of tree search — the branching ThoughtMap story (three parts under one root: how people think / what thinking is worth / do they agree). Formerly the `tree-search` deck; the old `lmcos-overview`/`main` deck was replaced by it. |
...
| `/figures/...` (absolute image `src`) | `src/public` → `../public`, then `public/figures` → `../..//human/figures` |
...
- **`vite.config.ts`** — widens `server.fs.allow` so the dev server can serve
  the `/human/figures` directory (outside `src/`).
...
Figures are served from the core `chess_analysis//human/figures`
directory via the symlink chain `src/public/figures → ../public/figures →
../..//human/figures`. Decks reference them with absolute `/figures/...`
...
```bash
python /human/move_time_summary.py
python /human/move_time_summary.py --include_zeroT
```
