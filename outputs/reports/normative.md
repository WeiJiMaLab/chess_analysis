...
> **Cross-corpus update (2026-07-08).** A robustness-checked replication on a second, independently-
> generated tree corpus (Leela/lc0 search on real 2023 human-game positions, filtered by a churn-motif
> quality filter) confirms this z_t-beats-both-baselines pattern with a *significant* paired margin, and
> survives the obvious "the fixed-stop baseline is right-censored" artifact check — see "Does the z_t edge
> replicate on an independently-generated tree corpus?" below. It is directionally corroborated (smaller
...
what the results mean and how sensitive they are to the assumed cost regime. A later section asks the
independent-replication version of the same question: does the same edge show up on a second, separately
generated tree corpus, and does it survive the obvious artifact checks?
...
independently generated** tree corpus, survives the most obvious artifact explanation, and corroborates
(or doesn't) on our own corpus once the same filter is applied. All numbers below use the same
architecture and training recipe as the rest of this report (`d_embed=32`, encoder pretrained 6 epochs,
PG readout trained 20 epochs, `time_mode=linear`) and the same paired-bootstrap significance methodology
(`cts.stats.bootstrap_ci`, percentile, `n_boot=2000`, never normal-theory).

### Does the `z_t` edge replicate on an independently-generated tree corpus?

The corpus is Yotam Sagiv's `human_trees`: 1,078,585 Leela/lc0-searched trees rooted at real 2023
human-game positions — generated by a different engine, a different search algorithm (lc0's own search,
not our PUCT), and a different population of root positions than our own corpus. A T1/T2-style sanity
panel (illegal-move, linear-chain, root-dominance, breadth-starvation, right-censoring checks) cleared
every red flag except a benign, explained duplicate-FEN pattern tied to this corpus's generation style
(every root expansion adds *all* legal children, not a PUCT-selective subset) — the corpus is
non-degenerate.
...
> **Result:** On an independently generated tree corpus (different engine, different search algorithm,
> different root positions), under a churn-motif quality filter, a learned `z_t` readout significantly
> beats both the best fixed stop and a hand-crafted tree-stats heuristic — the strongest confirmed
> positive result for `z_t` in this whole investigation, and the representational precondition
> (decodability) is even stronger here than on our own corpus.
...
— not just in the constructed cost regime on our own corpus, but on an independently generated corpus,
across a genuine (>10x) span of cost regimes, ruled out against the most obvious artifact explanation, and
directionally corroborated (smaller effect) on our own corpus under the same filter. That is real,
positive, robustness-checked evidence that a learned embedding can carry decision-relevant information a
hand-crafted 3-statistic readout does not. It is bounded on (at least) three axes that should not be
elided: the effect is specific to the `exclude_xaba` churn-motif filter (the standard argmax filter on the
identical corpus shows the opposite ranking), it fades to a tie once the fixed-stop optimum `k*` drops to
4 or below (mechanism not yet identified), and it has not yet been checked end-to-end on a from-scratch
regeneration of our own tree corpus under our own pipeline — that is a separate, not-yet-started effort.
...
readout 20 epochs, `pg_episode_batch=1024`), driven by `config_ysagiv_xaba.yaml` through the unmodified
pipeline (`slurm/pipeline/{pack_trees,train_encoder,pack_root,pack_root_merge,train_readout_pg}.slurm`).
...
- **The cross-corpus win is filter-specific, not corpus-specific.** On the identical ysagiv corpus, the
  ...
  `exclude_xaba` churn-motif filter, confirmed on two corpora now, not to the corpus or engine.
...
- **Not yet validated end-to-end on a from-scratch regeneration of our own corpus under our own pipeline.**
  Everything above either reuses ysagiv's independently-generated trees as-is, or applies the
  `exclude_xaba` filter post hoc to an already-generated `puctvalue_md36` population. Whether the effect
  survives a `exclude_xaba`-aware tree generation pass built specifically for our own pipeline (rather than
  a filter bolted on after the fact) is a separate, not-yet-started effort.
