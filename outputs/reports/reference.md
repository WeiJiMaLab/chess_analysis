...
| **Normative controller assessment** — does the learned `z_t` halt head beat hand-crafted tree-stats / the best fixed stop? Retracted 2026-07-06 (see R-TREEGEN-POISONED), then **revived** on a corrected corpus (first-play-urgency fallback fixed, greedy BeFS replaced with PUCT) — z_t significantly beats both baselines in an adequate constant-cost regime, and the win replicates when the position evaluator is swapped Stockfish→Leela on `human_trees` (Yotam Sagiv's corpus, generated by our own PUCT pipeline — not an independent one, see `ysagiv.md`'s correction). | [(R-EVALUATE)](normative.md) | Scientific | Deliberation | 📝 active; cross-corpus (evaluator-swap) robustness check confirmed, see [ysagiv.md](ysagiv.md) (`R-YSAGIV-XABA`) |
...
- **Faithful lc0-UCI `build_tree`**: this repo's own PUCT expansion (`c_puct=1.0`, unchanged default),
  querying the real `lc0` binary over UCI at `go nodes 1` as a one-shot policy+value evaluator ("faithful"
  = genuine lc0 network via UCI, **not** lc0's own internal search — `lc0` never runs its own MCTS here).
  Value = **raw value-head** (`win − loss`), budget **96** expansions, with the repetition-scan guard →
  **~11 s/tree** (A100; realized depth ~6). `edge_wdl_targets` included (the GNN child-WDL target).
- **Canonical set:** ysagiv `human_trees` — generated by this same `build_tree` pipeline with `lc0`
  (`t1-256x10-distilled-swa-2432500.pb.gz`) as the evaluator, over our own `fens.txt` root pool
  (**not** an independent root-FEN sample — confirmed with Yotam Sagiv 2026-07-09), `max_depth=30`,
  `min_nodes=max_nodes=96`; full generation config and correction: `normative.md`/`ysagiv.md`.
