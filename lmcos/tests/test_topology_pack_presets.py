...
from cts.train.gnn_pretrain import TopologyPretrainConfig, TopologyPretrainer
...
def test_pack_config_rejects_wide_without_supervision_shard() -> None:
    with pytest.raises(ValueError):
        PackPretrainConfig(topology_features=True, topology_supervision_shard=False)
...
def test_pack_config_maps_include_topology_targets_yaml() -> None:
    ...
    assert cfg.topology_features is False
...
def test_pack_config_maps_teacher_topology_yaml_to_full_template() -> None:
    ...
    assert cfg.topology_features is True
    assert cfg.topology_supervision_shard is True


def test_collate_topology_derives_targets_from_wide_node_features() -> None:
    schema = tree_encoder_feature_schema(topology_features=True)
    tail = torch.tensor([[0.5, 0.25, 2.0, 3.0]], dtype=torch.float32)
...
def test_topology_weighted_loss_masks_tail_dims(loss_type: str) -> None:
    model = TopologyModel(
        ...
        d_att=16,
        decoder_hidden=32,
        num_topology_targets=4,
    )
    cfg = TopologyPretrainConfig(loss_type=loss_type, topology_target_weights=(1.0, 0.0, 0.0, 0.0))
    trainer = TopologyPretrainer(model=model, device="cpu", train_examples=[], validation_examples=[], config=cfg)
    pred = torch.zeros(2, 4)
    targ_a = torch.tensor([[3.0, 9.0, 1.0, 4.0], [0.5, -2.0, 99.0, 0.01]])
    targ_b = torch.tensor([[3.0, -900.0, 0.0, 700.0], [0.5, 0.0, 0.0, 0.0]])
    loss_a = float(trainer._regression_loss(pred, targ_a))
    loss_b = float(trainer._regression_loss(pred, targ_b))
