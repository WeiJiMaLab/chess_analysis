"""Analyze how the controller's move preference evolves during oversearch.

For episodes where the controller halts later than the oracle (oversearch),
this script reconstructs the per-snapshot root Q-values along the search
trajectory and compares the move the oracle would have committed to at its
own stop step against the move the controller eventually commits to. The
outputs (plots, JSONL trajectories, summary report) feed a lab-notebook
investigation into whether continued search is changing the preferred move
or just refining a stable choice.
"""
...
class AnalyzeOversearchPreferenceEvolutionConfig(BaseModel):
...
# The CTS reconstruction stack pulls in torch, which may not be installed in
# the lightweight environment used for log-only re-summaries. Fall back to
# None sentinels so the script can still parse logs and plot halt-reward
# deltas when torch is unavailable; Q-reconstruction paths fail loudly later.
try:
    import torch
...
def load_diagnostics(path: Path) -> list[dict[str, Any]]:
    """Load the per-episode diagnostics JSONL file produced by greedy eval."""
...
def _load_trimmed_snapshot_q_maps(
    ...
    """Recover per-snapshot root Q-values and best moves for one example.

    Prefers the oracle trace cached on disk; falls back to recomputing it
    from the snapshot episode if the cache is absent. The "trimmed" prefix
    refers to truncating the episode at the first root-level decision so
    we only look at search steps before the move is committed.

    Returns:
        ``(q_maps, best_moves, final_root_q_values)`` where ``q_maps[i]``
        is the snapshot-local root-child Q dict at step ``i``,
        ``best_moves[i]`` is its argmax, and ``final_root_q_values`` is the
        terminal-snapshot Q dict used to score chosen moves at the end.
    """
    if (
        torch is None
    ...
    # Fast path: trust the precomputed trace baked into the example. Skips
    # an expensive teacher-search reconstruction when the dataset was
    # produced with traces enabled.
    ...
    # Slow path: rerun teacher search on each snapshot to obtain the root
    # child Q-values. Required for older datasets that don't store traces.
    if (
...
def _plot_halt_reward_delta_trajectory(diagnostics: list[dict[str, Any]], out_path: Path) -> dict[str, Any]:
    """Plot per-step halt-reward changes for oversearch episodes.

    For each oversearch episode and each search step ``t``, compute
    ``halt_rewards[t+1] - halt_rewards[t]`` and summarize the population.
    The resulting plot shows whether continued search tends to keep
    raising the controller's stated halt reward (the "preference" signal
    for oversearching) or not.
    """
    if plt is None:
    ...
    # Episodes have variable lengths; max_len is the longest delta sequence
    # any episode contributes (one fewer than the longest halt-rewards list).
...
def _collect_oversearch_preference_rows(
    ...
    """Materialize per-step + per-episode records for the oversearch cohort.

    For every oversearch episode, look up the snapshot Q-trajectory for its
    source tree, identify the "oracle move" (best at the oracle stop step)
    and the "metacontroller move" (best at the controller's later stop
    step), then record their per-step Q-values, availability, and the gap
    between them. Returns flat per-step rows for aggregation, per-episode
    summaries for plotting, and a status dict noting any missing sources.
    """
    oversearch = [episode for episode in diagnostics if int(episode["predicted_stop_step"]) > int(episode["oracle_stop_step"])]
    # Cache Q-maps per resolved source path: many diagnostics episodes
    # share the same source tree (different starting budgets), so loading
    # and reconstructing once per path is a big speedup.
    ...
    for episode in oversearch:
        ...
        # Defensive: if the reconstructed trajectory is shorter than the
        # diagnostics stop indices (rare; usually a sign of a stale cache),
        # drop the episode rather than index out of range.
        ...
        # Walk each search step within the episode and project the two
        # fixed moves through the changing per-snapshot root-Q dict.
        # ``None`` records steps where the move wasn't (yet) expanded.
...
def _summarize_relative_q(rows: list[dict[str, Any]]) -> dict[str, Any]:
    """Aggregate per-step rows by step offset relative to the oracle stop.

    Key 0 corresponds to the oracle stop step itself; negative keys are
    earlier search steps, positive keys are oversearch overshoot. Each
    bucket reports counts, availability fractions, and central tendency
    of the two Q values and their gap.
    """
...
def _same_move_regret_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Split the oversearch cohort by whether continued search changed the move.

    Reports episode counts, mean regret, and mean final-value gap for the
    two halves, and further breaks down move-switch episodes by whether
    the new move ended up better/equal/worse on the terminal-snapshot Q.
    """
    ...
    # 1e-9 tolerance to deal with floating-point ties from teacher search
    # without claiming every near-zero gap is a real preference.
...
def _plot_relative_q_trajectories(rows: list[dict[str, Any]], out_path: Path) -> dict[str, Any]:
    """Plot per-step Q-values (mean + 10/90 band) for oracle vs meta move.

    Two stacked panels: (1) the snapshot-local Q-values for each move,
    overlaid with the terminal-snapshot reference values; (2) availability
    fractions, which matter because both moves can be unexpanded at a
    given step. X-axis is step offset relative to the oracle stop.
    """
    if plt is None:
        raise RuntimeError("matplotlib is unavailable in this environment.")
    grouped: dict[int, list[dict[str, Any]]] = defaultdict(list)
    ...
    for x in xs:
        step_rows = grouped[x]
        # Per-step Q stats filter out unavailable moves; final-value and
        # availability stats include all rows in the bucket.
...
def _plot_relative_q_gap(rows: list[dict[str, Any]], out_path: Path) -> dict[str, Any]:
    """Plot the signed Q-gap (meta - oracle) per relative step.

    Companion to ``_plot_relative_q_trajectories``: collapses the two
    Q-curves into a single preference signal. A positive band means the
    controller's eventual move looks better than the oracle's at that
    step; negative means the opposite.
    """
    if plt is None:
        raise RuntimeError("matplotlib is unavailable in this environment.")
    grouped: dict[int, list[dict[str, Any]]] = defaultdict(list)
    for row in rows:
        # Drop rows where either move is unavailable — the gap is undefined.
...
def _write_report(path: Path, summary: dict[str, Any]) -> None:
    """Render a small human-readable Markdown summary alongside summary.json."""
    lines = [
...
def main(config: AnalyzeOversearchPreferenceEvolutionConfig) -> None:
    """CLI entry point. See ``--help`` for the full argument list.

    Outputs land under ``--output-dir``:
      - ``oversearch_halt_reward_delta_trajectory.png`` (always, if mpl available)
      - ``oversearch_preference_q_trajectories.jsonl`` (per-episode records)
      - ``oversearch_choice_q_trajectories.png`` (Q curves + availability)
      - ``oversearch_choice_q_gap.png`` (signed preference gap)
      - ``summary.json`` + ``report.md`` (aggregate stats)
    """
    ...
    path_rewrites = _parse_path_rewrites(list(config.path_rewrite))

    # Halt-reward deltas only need the diagnostics JSONL; emit them
    # unconditionally so this plot is available even when source trees
    # are missing and Q reconstruction is skipped.
    ...
    oversearch = [episode for episode in diagnostics if int(episode["predicted_stop_step"]) > int(episode["oracle_stop_step"])]
    # Probe the filesystem once up front to classify each source tree as
    # locally available or missing; drives both the gating below and the
    # status block written into summary.json.
    resolved_paths = {
    ...
    # Fail loud if the user did not opt in to a partial run. Common case:
    # forgot a --path-rewrite when running on a workstation that doesn't
    # mirror the cluster's absolute paths.
    if source_status["missing_source_paths"] and not config.allow_missing_sources:
...
if __name__ == "__main__":
    ...
    run_with_config_cli(AnalyzeOversearchPreferenceEvolutionConfig, main)
