...
def main() -> None:
    ...
    ap.add_argument("--gnn-regret", type=float, default=None,
                    help="tier-5 Readout(GNN-z) mean regret; fills its slot (else 'pending')")
    ap.add_argument("--gnn-stop", type=float, default=None,
                    help="tier-5 Readout(GNN-z) P(stop==OSS); fills its slot (else 'pending')")
    args = ap.parse_args()
    ...
    # Tier 5 (GNN-z): real when supplied, else 'pending' (awaits the P2 encoder).
    pending = args.gnn_regret is None
    regret_full = regrets + [args.gnn_regret if not pending else 0.0]
    stop_full = stop_acc + [args.gnn_stop if not pending else 0.0]
    if not pending:
        print(f"{labels[4]:<26s} {args.gnn_regret:>10.4f} {args.gnn_stop or 0.0:>14.3f}")
