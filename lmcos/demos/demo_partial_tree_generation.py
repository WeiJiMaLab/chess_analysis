...
from cts_pretrain import NodeBudgetDistribution, TeacherSearchConfig, generate_partial_tree_from_provider
...
def render_tree_svg(tree: SearchTree, out_path: Path) -> None:
    ...
    canvas_h = int(margin_y + (max_depth + 1) * depth_step_y + node_h)
    ...
    for node in tree.iter_nodes():
        ...
        value = float(node.scalar_features.get("value", 0.0))
        header = f"move: {move}   value: {value:.3f}"
        ...
        svg.append(
            f'<text x="{x + node_w / 2:.1f}" y="{y + 24:.1f}" text-anchor="middle" font-family="Inter, Helvetica, Arial, sans-serif" font-size="13" fill="#0f172a">{escape_xml(header)}</text>'
        )
        svg.append(
            f'<line x1="{x + 8:.1f}" y1="{y + 34:.1f}" x2="{x + node_w - 8:.1f}" y2="{y + 34:.1f}" stroke="#e2e8f0" stroke-width="1"/>'
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Demo: build a budgeted partial tree using provider + PUCT, then render as SVG.\n"
    ...
    render_tree_svg(generated.tree, out_svg)
