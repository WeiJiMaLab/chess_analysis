...
Index of all reports (including **open procedure steps**): [reports/reference.md](reports/reference.md).  
Older lmcos work (Apr–May 2026) lives in the [§ Legacy lmcos lab notebook](#legacy) at the bottom.

---

## ⚙️ Running state (as of 2026-06-18) {#running-state}

> **Move-time analysis settled on the canonical `human_trees`; metric is GSS (greedy stopping step); VOC renamed → Gain. Branching dominates human RT; a resource-rational account is drafted ([R-BRANCH](reports/branching.md)).**

- **Canonical trees:** ysagiv `human_trees` (lc0 search on 2023 human-game root FENs, 4-field; ~114K, still growing). The local 150K `lc0_trees` (a lexicographic slice of `fens.txt`) was **non-representative** (low-diversity / off-distribution) and **deleted** — generation was identical (same net/config; same-FEN trees agree ~5e-3), so it was a FEN-population artifact, not a bug. **Join by 4-field FEN, never by filename index.**
- **Pipeline:** `human_analytics/tree_values_analysis.py` derives **GSS / Gain / Action-gap / MQ** per tree, caches per-tree values to parquet (re-plots need no recompute), and joins to human RT by 4-field FEN. Lean slurm `tree_values.slurm` (16 cpu / 64 G / 30 min) for the one-time compute; **CPU-only by design** (reads pre-generated trees, no lc0 inference).
- **Metric (GSS):** greedy stopping step = first expansion the eventual-best move is found (the budgeted oracle's *zero-cost* stop) — **not** the cost-aware DP `optimal_stop_step` (default λ=18.5 bails at step 1). r(GSS, logRT)=**+0.115**; Gain +0.073; MQ −0.154; action gap −0.064. **Branching ρ≈+0.35 dominates** every engine metric.
...
> **Two more real bugs found in the packhistory retrofit and fixed: a train/inference topology mismatch (a "compute-saving" filter was silently corrupting attention over kept sibling edges) and a dormant `root_rank` step-scale bug. Relaunched `xaba100k_minply15_maxply75` end-to-end post-fix. Got the two most load-bearing regression tests (`test_diff_vs_old_mc_packed`, `test_no_future_leakage.py`) running for real again — they'd silently gone SKIPPED after the `xaba20k` data they were pinned to was purged.**

| Description | Rationale | Status / finding | Reference |
|---|---|---|---|
| **Train/inference topology mismatch found and fixed**: `pack_history.py`'s "never-visited-leaf filter" (a 2026-07-10 compute-saving optimization, ~88.8-97.7% of nodes per tree) dropped never-visited children from `edge_parent`/`edge_child`/`edge_slot` before returning a training example — but those arrays are the literal graph `TreeEncoder`'s segmented-softmax attention runs over, so dropping a child changed the softmax denominator (and therefore the prediction) for every *kept* sibling under the same parent. Production inference never applied any such filter | A skeptical leakage re-audit, not the original design review, caught this | ✅ Fixed: `build_snapshot_pair_example` now passes T_n's full unfiltered edge set through; never-visited edges naturally get weight=0 via the pre-existing Δvisits machinery (nothing downstream needed to change). Compute cost: ~14.6x more candidate edges per shard, accepted as a correctness-over-compute tradeoff | `src/cts/data/preprocess_gnn/pack_history.py::build_snapshot_pair_example` |
| **`xaba100k_minply15_maxply75` relaunched** post-fix (jobs 10968362-66, full 5-stage chain, `--dependency=afterok` chained) after cancelling and purging an earlier pre-fix partial run | First real production-scale confirmation of the future-leakage + topology fixes on a second dataset, not just `xaba20k` | ✅ All 5 stages `COMPLETED` clean | `config_ysagiv_xaba100k_minply15_maxply75_history.yaml` |
| **`root_rank` step-scale bug found (dormant) and fixed**: `pack_history.py`'s value queries (`_build_tree_n`, `build_snapshot_pair_example`'s target + Δvisits window) read the update log at the *trimmed* (`root_rank`-relative) step scale, but the log itself is packed on the *full*, untrimmed scale — structure was always correct, values would have been stale by exactly `root_rank` steps whenever `root_rank > 0`. Found independently by two parallel review agents | Routine post-hoc audit (3 subagents, no specific complaint) rather than a reported symptom — `root_rank == 0` on every real tree checked (a fresh single-root search always expands the root first), so this never actually corrupted real training data, but nothing enforced that invariant | ✅ Added `_full_scale_step` helper, routed all three queries through it; converted the test that only *documented* the bug into a real regression test (`test_value_query_respects_root_rank_fix`) | `src/cts/data/preprocess_gnn/pack_history.py::_full_scale_step` |
| **Got `test_diff_vs_old_mc_packed` + all 5 `test_no_future_leakage.py` tests running for real again** — both had silently gone `SKIPPED` (not failing, just not running) after `ysagiv_xaba20k/pack_history/` was purged in an earlier buggy-run cleanup; `test_diff_vs_old_mc_packed` had only ever passed once, before either fix above landed | These are the most direct regression checks for the whole retrofit effort — worth noticing they'd gone quiet rather than assuming "no failures" meant "still passing" | ✅ Repointed both test files at `xaba100k_minply15_maxply75` (already has both old+new baselines on disk, fresher than `xaba20k`'s ever was). Two of the file's hardcoded fixtures (`episode_index=8, node_id=66`) didn't transfer to the new corpus — rewrote to dynamically discover a valid fixture instead. Two false-alarm failures along the way, both resolved as test-harness bugs, not real regressions: (1) discovery picked a node visited exactly once, so "value should keep evolving" trivially failed — fixed by requiring ≥2 update-log entries; (2) a test recomputed expected variance from `q_value` instead of production's actual `p_win − p_loss` formula, leaving ~1e-4 slack that just missed a 1e-4 tolerance — fixed by matching the formula (and tightening tolerance to 1e-6, since it's now the same computation). Final: 12/12 real-data tests pass | `src/cts/tests/test_no_future_leakage.py`, `src/cts/tests/test_packhistory_trees.py::test_diff_vs_old_mc_packed` |
| **Flagged, not fixed: a versioning landmine** — `preprocess_mc/pack.py` was rewritten in place to serve both the old `mc_pack` stage and the new `packhistory_trees` stage from one module, unconditionally emitting the new shard format. The frozen `mc_packed/` baseline `test_diff_vs_old_mc_packed` diffs against is frozen only because nobody has re-run `mc_pack` since the rewrite — a stray re-run would silently overwrite it with no error | Found by the same audit pass; deliberately not fixed this session (needs either a hard stage-name/format guard or splitting the old code path into its own frozen module) | ⬜ open | `history.md` § Deferred |
| **`history.md` condensed** — the 700-line six-task-parallel-agent planning/progress-log document (Wave 1-3 DAG, per-task dated Progress Log, Definition-of-Done checklists) rewritten down to current-state-only (design, parameters, file map, validated results, open items); the narrative/debugging trail preserved here instead, per this repo's existing convention (`mc_pipeline`-style current-state docs vs. `labnotebook` for history/diagnoses) | User: clean up stale info, only record what worked | ✅ | `history.md` |

## 2026-07-10 {#2026-07-10}

> **Diagnosed and fixed the meta-controller's core data bug: per-node value/WDL features were tensorized once from the complete 96-step tree and never updated per step, so the controller was training on a frozen final answer at every decision point regardless of how much search had actually happened. Fixed via retroactive backprop replay (no re-running the engine) + a new k-steps-ahead GNN pretraining objective on partial-tree snapshots, reusing `ChildWdlModel` unmodified. Six-task parallel effort, ran end-to-end on `xaba20k`, then confirmed the fix is a real improvement (not just mechanically correct): `z_t` beats a simple action-gap stopping heuristic in 10/10 cost regimes tested, all statistically significant.**

| Description | Rationale | Status / finding | Reference |
|---|---|---|---|
| **Bug found and proven on real data**: root children with a real, large (0.3-0.6) backed-up-value swing across a 96-step trajectory had bit-identical packed encodings at the start and end of that swing — the packed action-gap was therefore constant across all steps while the true oracle action-gap moved substantially, sometimes flipping which move looked best | The MC's whole job is deciding whether more search would change its mind; a value that never changes removes the one signal most relevant to that decision | ✅ Diagnosed via direct comparison against `oracle_root_q_trace` | `history.md` § What's wrong |
| **Fix mechanism: retroactive backprop replay** — every raw tree already stores final structure + each node's static leaf eval + real expansion order, so replaying MCTS backprop bookkeeping (no engine re-inference) recovers the true per-step backed-up value/WDL for every node, packed as a sparse per-edge update log with O(log M) forward-fill lookup | Recover step-accurate values without regenerating any tree | ✅ Replayed root-child Q vs. the tree's own stored `oracle_root_q_trace`: mean error 3.3e-5, 0.16% of comparisons exceed 1e-3 (residual traced to unrecoverable terminal-leaf-revisit backprop mass, not a bug) | `src/cts/data/preprocess_mc/pack.py::_replay_backprop_history` |
| **New GNN pretraining objective**: sample one random step `n` per tree, build `T_n` with real step-`n` values, supervise every edge's k-steps-ahead (`k=12`, v1, not tuned) WDL, weighted by Δvisits. An initial design added a new `v_n`-augmented head (`FutureWdlHead`) reasoning that `ChildWdlHead` only worked for zero-history children — **user caught this as wrong**: `ChildWdlHead` already works on richly-explored children today via bottom-up message passing before the head ever sees it | Avoid unnecessary new model surface if the existing head already generalizes | ✅ Reverted `FutureWdlHead`; reused `ChildWdlModel`/`ChildWdlHead` unmodified on `T_n` instead. ⚠️ But the rigorous validation test built to confirm the mechanistic claim found it only half holds: two siblings' predictions do carry real, distinguishable signal, but perturbing one sibling's subtree also moves its *untouched* sibling's prediction by nearly the same amount (ratio ≈0.98-1.03, reproducible, not a training-budget artifact) — real cross-sibling leakage in `ChildWdlHead`, plausibly pre-existing (not proven T_n-specific), **deferred by explicit user call**, not fixed | `src/cts/tests/test_child_wdl_disambiguation.py` |
| **Wave 3 ran end-to-end on `xaba20k`**: `packhistory_trees` → `packhistory_GNNpretrain` → `train_encoder` (loss 0.14→0.036) → `packhistory_MCmaterialize` → `train_readout_pg` (`val_greedy_regret` 0.108→0.061) | Validate the full new pipeline on real data before trusting anything downstream | ✅ All 5 stages `COMPLETED` clean; diff test vs. the old frozen-value `mc_packed/` output confirmed structure/oracle-values byte-identical, per-step node features now genuinely differ at every step | `history.md` § Wave 3 |
| **Sub-investigation: does the fix actually help, or just run?** User specifically asked whether `z_t` beats the trivial action-gap stopping heuristic, not just whether the pipeline completes | A trained controller finishing isn't the same as the fix being an improvement | ✅ Matched-procedure comparison (same PG training loop, same regimes): `z_t` wins in **10/10 cost regimes** tested, all statistically significant (paired bootstrap CI excludes 0); R-decodability `z_t` R²=0.619 vs. action-gap 0.289; frontier regret `z_t`=0.0247 vs. AG=0.0307 (non-overlapping CIs). One open, partial finding: `z_t`→action-gap recoverability is real but lossy (R²=0.58-0.68, not ~1.0) | `outputs/figures/ysagiv/xaba20k_history/{frontier,decodability,delta_regret}_data.json` |
| **Real future-leakage bug found** (separate from the frozen-value bug this whole effort fixes): nodes not yet individually backed up as of a queried step were reading their *final* end-of-search value instead of zero, in both `controller_train.py` and `pack_history.py`; a second instance in `wdl_var` specifically | Found while preparing the `xaba100k_minply15_maxply75` relaunch | ✅ Both fixed (zero-init instead of clone-from-final-value); new regression suite `test_no_future_leakage.py` (5 tests) + adversarial canary/poisoning suite `test_canary_future_leakage.py` (7 tests) added, all passing | `src/cts/train/controller_train.py`, `src/cts/data/preprocess_gnn/pack_history.py` |

## 2026-07-08 {#2026-07-08}

> **PUCT promoted from comparison variant to the pipeline's default tree-gen path (BeFS demoted to `variants.befs`); added a non-destructive `argmax_filter` stage ("thinking helps" subsetting, argmax>2) ahead of `pack_trees`; widened the raw tree sample 250K→400K. Full overnight preprocess→eval chain ran clean through training (best `val_greedy_regret=0.1162` after 20 PG epochs) but `eval` crashed on a fresh bug — a z_t cache/episode ordering mismatch — unresolved as of now.**

| Description | Rationale | Status / finding | Reference |
|---|---|---|---|
| **PUCT made the default `treegen.selection`** (was `befs`); `value_feature: cp_order`→`value`; output dir `befs1cp_md36`→`puctvalue_md36` everywhere (`human_analysis.trees_default`, `split.source_root`); BeFS demoted to a new `variants.befs` comparison fork | 2026-07-07 decided PUCT is the live path for this branching regime — make that decision load-bearing in config, not just a variant override | ✅ Config flip (uncommitted); BeFS still runnable via `RUN_VARIANTS=befs` for a PUCT-vs-BeFS comparison on the same tree roots (`share: [fens_sample]`) | `config_minply15_maxply75.yaml` |
| **New `argmax_filter` pipeline stage** (`gen_trees → argmax_filter → pack_trees`): non-destructively keeps only trees where the TRUE oracle argmax stop step exceeds 2 (`cts.data.preprocess_mc.filter_argmax`), writing an include list `split.py` reads (`split.include_list`) — `gen_trees`'s own output and `engine_analysis_pwin/cp` still see the full unfiltered population | On the unfiltered population, SingleHalt* collapses to a near-zero stop; the argmax>2 "thinking helps" subset is where a meta-controller has something to learn (900-FEN pilot: ~24.7% yield, non-trivial k*=7) | ✅ `slurm/pipeline/argmax_filter.slurm` (1 cpu/8G/3h, ~0.016s/tree single-threaded, ≈107min at 400K); `human_analysis.tree_fens_n` 250000→400000 so the post-filter survivor pool (≈100K) stays generous after the ~75% cut | `slurm/pipeline/argmax_filter.slurm` |
| **`mc_pack`/`train`/`eval` `maintenance_exponent` 1.1→1.0** (exactly linear in node count, was slightly superlinear) | Simplify the cost model now that `time_lambda`/`maintenance_scale` are independently calibrated (2026-07-07) | ✅ | `config_minply15_maxply75.yaml` |
| **★ Full overnight pipeline relaunch** on the new PUCT-primary config (`gen_trees` 160-task array → `engine_analysis_pwin`/`cp` → `argmax_filter` → `pack_trees` → `train_encoder` → `pack_root` 40-task array → `pack_root_merge` → `train_readout_pg` → `eval`) | Validate the PUCT-primary + argmax-filter + 400K config end to end before trusting any downstream number | ✅ Every stage through `train_readout_pg` (job 10805130) `sacct`-confirmed `COMPLETED`, exit 0:0 — PG training itself looks healthy (`val_greedy_regret` 0.85→**0.1162** over 20 epochs, `val_stop_acc` 0→0.046, monotone improvement, no saturation this time). ❌ **`eval` (job 10805131) FAILED**, exit 1:0, 10s in: `ValueError: z_t cache/episode misalignment — validation cache must be shuffle=False.` (`_load_zt_by_episode`, `evaluate.py:168`) — the concatenated per-episode `tree_sizes` don't match the `n_t` column read from the z_t feature-cache shards, i.e. the cached z_t shard order and the freshly-loaded validation-episode order have drifted apart. **Not yet diagnosed or resubmitted** — no eval job after 10805131 as of writing | `src/analysis/evaluate.py:168`, `slurm/logs/eval_10805131.err` |
| **Open**: (1) fix the z_t cache/episode shuffle-alignment bug and get a clean default (PUCT-primary, argmax-filtered, 400K) `eval` run; (2) once clean, run `variants.befs` for the PUCT-vs-BeFS comparison this config now sets up; (3) `outputs/reports/normative.md` / R-EVALUATE still has **no report** — neither the 2026-07-07 z_t-beats-baselines result nor this run's config are written up yet | — | ⬜ not started | `reference.md` R-EVALUATE row |

## 2026-07-07 {#2026-07-07}

> **Overnight BeFS+PUCT run finished clean (no crashes), but `eval.py`'s regret/frontier results were degenerate — `time_lambda=10.0` was ~1000-3000x too large for the now-win-probability-scaled reward, so 100% of episodes trivially "stop at step 0." Three subagent sweeps calibrated `time_lambda`, checked whether filtering could remove the residual zero-cost floor (it can't, and shouldn't — it's real data), and confirmed convex cost shapes (quadratic/power_law) buy nothing over linear. Decided PUCT is the live path (not BeFS). Found `oracle_value` doesn't need to be baked in at packing time at all — `pg_controller_train.py` now derives it fresh as `max(g)`, verified byte-identical to the old baked value on real data, so `time_lambda` changes never require repacking again.**

| Description | Rationale | Status / finding | Reference |
|---|---|---|---|
| **Checked in on the overnight run**: all base (BeFS) + `puct` variant jobs `sacct`-confirmed `COMPLETED`, exit 0:0, across both full chains | Confirm no crashes before trusting any output | ✅ Clean. But `eval.py`'s `delta_mean_regret_lambda`/`frontier` plots showed every method (`SingleHalt*`, `Stats-Controller`, z_t) landing at **exactly** `+0.0` vs. baseline, identical to a trivial `always_stop` policy — not a rounding artifact (confirmed at full float precision, 0/600 episodes nonzero) | — |
| **Root cause**: `curve[s] ≈ halt_reward[s] − s·time_lambda`; with `time_lambda=10.0` and `halt_reward` now on a win-probability scale (±1-2), cost swamps signal from step 1 onward — confirmed by hand: `curve[0] = [-0.534, -10.534, -20.534, ...]`, argmax@step0 for 100% of 200 sampled episodes | Same class of unit mismatch as the `min_halt_reward_range` bug from 2026-07-06 (both assumed the old cp-scale reward) | ✅ diagnosed via direct code trace + numeric reproduction, not inference | — |
| **3 parallel subagent sweeps on real packed PUCT/BeFS episodes**: (1) `min_halt_reward_range` filter — turned out to already be working correctly all along (`tree_strata_filtered=0` is a naming collision with an unrelated disabled feature; real filtering matches observed `skipped` counts almost exactly: BeFS 68.5%, PUCT 35.1% at 0.05). (2) `time_lambda` calibration — direct sweep found **BeFS≈0.003, PUCT≈0.01** (range 0.001-0.03) give a non-degenerate stopping-step spread; both ~1000-3000x smaller than 10.0. (3) Convex cost shapes (`time_mode="quadratic"`/`"power_law"`) — no shape beats a well-calibrated linear cost once each is tuned; `power_law` at the shipped default `p=2.8` is nearly insensitive to λ across 7 orders of magnitude (bad knob) | Calibrate the cost model from data, not guesswork; check whether "most episodes stop immediately" is fixable by filtering | ✅/⚠️ Recommended `time_lambda=0.01` (PUCT). Filtering does **not** fix the residual ~17-20% "peaks at step 0 even at zero cost" floor (flat 14-20% across `min_halt_reward_range` 0.05-0.5; `min_decision_margin` is either a no-op at the old λ or destroys >99% of data at any nonzero threshold) — and it shouldn't: that floor is genuine positions where stopping immediately is correct, not contamination. The actual test for "did we train a trivial always-stop network" is post-training (does the controller's predicted-stop rate vary with z_t/tree-stats), not a pre-packing filter | — |
| **Decision: PUCT is the live path**, not BeFS — better suited to this project's branching regime, no exploration term is the wrong default for chess-scale trees | User call, informed by the above (BeFS's own tunnel-vision inflates both the zero-range-tree rate and the zero-cost step-0 floor relative to PUCT) | ✅ | — |
| **`train_readout_pg.slurm` crashes** (`_validate_packed_manifest_oracle` rejects the new `time_lambda` against the packed manifest's old baked-in value) **left as-is, on purpose** | Traced who actually reads its output checkpoint (`mchalt_controller.pt`/`controller_checkpoint`): nobody. `grep` across `src/` found zero non-log references. `evaluate.py` imports `fit_readout_pg` directly from `pg_controller_train.py` and does its own fully self-contained readout fit per regime (`fit_readout_pg`'s own docstring: *"The `analysis.evaluate` path — no encoder / cache / checkpointing"*) — it never touches the saved checkpoint. `pipeline.yaml`'s `eval: after: [train_readout_pg]` dependency is therefore stale, predating some earlier refactor | ✅ Decision (not a bug fix): don't relax the validation or fix this stage — it produces an artifact nothing consumes. Resubmitted `eval@puct` directly with no dependency instead of waiting on/fixing `train_readout_pg` | `src/analysis/evaluate.py:49`, `src/cts/train/pg_controller_train.py:191-214` |
| **Deleted the stale base-run (BeFS) `normative/` figures** (`outputs/figures/minply15_maxply75/normative/`) — degenerate (old λ=10.0) and for a no-longer-live algorithm | User: confusing to have around | ✅ removed from disk (left git index alone — that dir had pre-existing staged content from before this session, unrelated, not touched) | — |
| **First `eval@puct` re-run (job 10779091)**: `frontier`/`R-decodability` now non-degenerate (`SingleHalt*(k=1)=0.1 Stats=0.1 z_t=0.1`, paired z_t−Stats CI (−0.0054,+0.0018) straddles 0; decodability z_t=0.285 vs steps=0.076) — the actual fix from this whole investigation working correctly | Confirm the `time_lambda=0.01` fix produces sane results, not just non-crashing ones | ✅ frontier/decodability fixed. ⚠️ `delta_mean_regret_lambda`/`maintenance` plots STILL fully degenerate (`+0.0` everywhere but `always_continue`) — different root cause: `plot_delta_regret_vs_zt`'s `lambda_grid=(5,10,40)` and `maint_grid=(0,0.05,0.10,0.30,1.0)`/default `maint_lambda=10.0` are hardcoded function-signature defaults, separate from `eval.time_lambda`, never touched by the earlier fix. Also: `eval.maint_lambda` config field (10.0) was a sibling I'd missed updating alongside `eval.time_lambda`. Job also `TIMEOUT`'d at 45:10 against a 40min limit, but log shows clean `EVAL DONE`/exit 0:0 — finished right at the wall-clock edge | — |
| **Recalibrated `lambda_grid`/`maint_grid`/`eval.maint_lambda`** | Same rigor as the `time_lambda` fix — no guessing | ✅ `eval.maint_lambda` → `0.01`. `lambda_grid` → `(0.003, 0.01, 0.03)`, reusing the already-validated PUCT `time_lambda` sweep (no new data needed). `maint_grid` → `(0.0, 0.003, 0.01, 0.03, 0.1)`, from a NEW dedicated real-data sweep (`time_lambda` held at 0.01): frac-argmax@step0 goes 0.278→0.297→0.325→0.365→0.505 across the grid (0.968 at maint=1.0 — confirmed as the saturation point to avoid, mirroring old grid's collapse). Bumped `eval.slurm`'s time limit 40min→55min (a correctly-calibrated run does real fitting work instead of instantly collapsing, so it's genuinely slower). Resubmitted `eval@puct` (job 10782191) | `src/analysis/evaluate.py::plot_delta_regret_vs_zt`, `config_minply15_maxply75.yaml` |
| **`oracle_value` doesn't need to be baked in at pack time at all**: `pg_controller_train.py`'s PG loss (`_expected_regret`) computes `loss = oracle_value − E[return]`; `E[return]` was already recomputed fresh from raw `halt_rewards`/`tree_sizes`/`time_budgets` via `return_for_stop_step`, but `oracle_value` was read from `payload["oracle_values"]`, precomputed by `compute_budgeted_oracle` at `mc_pack` time under whatever `time_lambda` was in effect then — mixing a fresh-λ return against a stale-λ baseline | Realized (user's instinct, correctly) that cost params shouldn't need to leak into packing at all | ✅ Since this is a fully-observed, deterministic stopping problem, `oracle_value` is provably `max_s(return_for_stop_step(s))` — exactly `max(g)`, already being computed. Fixed `_episode_returns` to derive `ov` this way instead of trusting `em.oracle_value`. Verified on 1000 real PUCT validation episodes at the *current* baked λ=10.0: `max(g) == em.oracle_value` exactly, 0 mismatches, `max_abs_diff=0.0` (script: scratch `verify_oracle_value_equiv.py`). Matches an existing pattern already used by `_aggregate_greedy_rollout_metrics`'s `live_oracle_value` recompute, just not previously applied to the PG training loss. **Consequence: `time_lambda`/cost-model changes never require a repack again** — only `mc_pack`'s episode-selection filters (`min_halt_reward_range`, `min_decision_margin`) actually depend on what's baked into the packed shards | `src/cts/train/pg_controller_train.py::_episode_returns` |
| **Frontier plot had another leftover cp-scale assumption**: axis padding was fixed absolute constants (`±20/25/30`) sized for when regret was in the tens-to-hundreds; against the new ~0.1-scale regret they forced a ±25-30 range around data that only spans a few tenths, making the real spread invisible | User: axis range "doesn't make any sense," should depend on the data's own spread | ✅ Added `_padded_range(lo, hi, frac, min_pad)` -- proportional padding, scale-agnostic by construction (won't break again if the reward scale changes a third time). Also decluttered `delta_mean_regret_lambda`/`maintenance`: `always_stop`/`always_continue` are routinely degenerate (huge or exactly-zero) and forced the x-axis to their range; since `SingleHalt*` is fit by `argmin` over every possible fixed stop step (`fit_singlehalt_stop`), including k=0 (=Always Stop) and k=max (=Always Continue) as candidates, it's guaranteed at least as good as either on the fit split — so only `SingleHalt*`/`Stats-Controller` are plotted now (`_DELTA_CANDIDATES_DISPLAY`); all 4 are still computed/printed in `[assess]` lines | `src/analysis/evaluate.py::_padded_range`, `_DELTA_CANDIDATES_DISPLAY` |
| **Refactored all 3 plot functions into compute/render halves** + save/replot | Every plot fix tonight required a full ~45min eval rerun (data load + model fit) just to change axis padding or which candidates to show — expensive rerun for a cosmetic change | ✅ `plot_regret_effort_frontier`/`plot_r_decodability`/`plot_delta_regret_vs_zt` split into `_compute_*_data` (expensive: load+fit, JSON-serializable output) + `_render_*` (cheap: build+save figure). Each `plot_*` wrapper saves its compute output to `<out_dir>/{frontier,decodability,delta_regret}_data.json` (via new `_to_jsonable`/`_save_json`/`_load_json`). New `replot_saved(out_dir, which)` + `--replot` CLI flag re-render from saved JSON, skipping data load/model fit entirely | `src/analysis/evaluate.py::replot_saved`, `main()` |
| **Retraining-free "headroom" sweep**: is there a (λ, maintenance_scale) region where a learned/input-conditioned halter has real theoretical room to beat `SingleHalt*`? | User: estimate this without retraining, want a recommendation for the baseline | ✅ `SingleHalt*`'s regret vs the TRUE per-episode oracle needs zero model fitting (`fit_singlehalt_stop` is a pure argmin over `_regret_at`, itself pure arithmetic on raw `halt_rewards`) — swept 8×7 (λ×maint) grid on 5000 real PUCT episodes in seconds. Fraction of achievable regret already recovered by the best fixed-k: **0.83-0.94 at maint≤0.03** (little room left); **0.68-0.77 at maint≈0.1** (real gap); smooth falloff to 0 by maint=0.3 (full collapse) — confirmed with a finer follow-up sweep at λ=0.01 (maint 0.1→0.3: 0.726→0.583→0.391→0.145→0.000). Combined with the ALREADY-known, cost-regime-independent decodability result (z_t R²=0.285 vs steps-only 0.076 for `R(t)`, which has no cost term at all) -- z_t carries real signal, so wherever headroom is large that signal should translate to lower regret, not just decodability on paper. **Chosen sweet spot: `eval.maintenance_scale=0.1`** (λ stays 0.01) for the frontier's headline numbers -- ~73% recovered, real room without being degenerate. `maint_grid` widened to 7 points (`0.0, 0.01, 0.03, 0.1, 0.15, 0.2, 0.3`) to show the whole transition, not just the first part of it | scratch `headroom_sweep.py`; `config_minply15_maxply75.yaml` (`eval.maintenance_scale`), `src/analysis/evaluate.py::plot_delta_regret_vs_zt` (`maint_grid`) |
| **★ Final `eval@puct` re-run (job 10789578, 1h30 limit -- prior two runs at 55min/40min timed out mid-sweep, not stuck, just legitimately slower with the corrected non-degenerate lambda + bigger maint_grid) — z_t SIGNIFICANTLY beats both `SingleHalt*` and `Stats-Controller`**, growing monotonically across the whole maintenance sweep: Δregret (z_t advantage, mean±95%CI, both baselines identical) = maint 0: +0.0017(n.s.) · 0.01: +0.0048(sig) · 0.03: +0.0083(sig) · **0.1: +0.0331 (+0.0277,+0.0387), matches the frontier's paired result exactly** · 0.15: +0.0595(sig) · 0.2: +0.0763(sig) · 0.3: +0.0977(sig). Lambda sweep same pattern in miniature (0.003/0.01 n.s., 0.03 sig: +0.0067). Full-precision numbers pulled straight from the saved `delta_regret_data.json` (via `bootstrap_ci` on the raw per-episode arrays) since the `[assess]` print line's `{:+.1f}` rounds everything in this range to `+0.0`/`+0.1` -- would have looked flat/uninteresting at print precision alone | This is the actual research question the whole `R-EVALUATE` investigation was chasing: does z_t meaningfully beat hand-crafted tree-stats/fixed-stop at deciding when to halt | ✅ **Yes, and not just at the chosen sweet spot** -- even at maint=0.3, where `SingleHalt*` is FULLY collapsed (0% of achievable value recovered per the headroom sweep), z_t still recovers roughly half of the (smaller but nonzero, ~0.18) achievable value -- the real signature of an input-conditioned policy correctly identifying which individual episodes are still worth continuing, something no single fixed threshold can do by construction. Confirms the retraining-free headroom methodology correctly predicted where the effect would show up | `outputs/figures/minply15_maxply75/variants/puct/normative/{frontier,delta_regret}_data.json` |
| **Config**: `mc_pack`/`train`/`eval` `time_lambda` all set to `0.01` (mc_pack's copy is now vestigial for training, kept only so it's not misleadingly stale if `min_decision_margin` is ever enabled) | Apply the calibration | ✅ | `config_minply15_maxply75.yaml` |
| **Human** `board.md` follow-up: material-imbalance dips and the checks-available inverted-U both re-diagnosed as compositional (population-mixture) artifacts, not new mechanisms | Push the `f64f618` simplification's "why" further with supplementary confound-removed figures rather than prose-only argument | ✅ Signed material imbalance's −3/−9 dips vanish entirely under recapture exclusion (`supp_material_imbalance`, n=66.77M) — a `prev_move_was_capture` composition artifact. `abs_material_imbalance`'s \|3\|/\|9\| spikes: `prev_move_was_capture` share hits 49.6%/74.1% there vs 30–66% at neighbors (recapture positions 3.63s vs 7.50s) — \|3\|/\|9\| are the only magnitudes reachable by a single clean piece-for-piece capture (pawn/minor/rook/queen = 1/3/5/9). Checks-available inverted-U: a 3-way material-band split (≤−5/0/≥+5) shows it's a population mixture, not a within-population effect — compositional shift alone explains ~40% (early game) to ~80% (tail) of the pooled decline; a total-pieces tertile split within the material-even band shows dense boards never decline | [board.md](reports/board.md) |
| **Human** new `opponent_mobility` axis + queen-retention/king-edge-distance mechanism figures for the checks-available "way ahead" material band | Push that band's story past correlation into mechanism | ⏳ Sample-based only so far, not yet a full-table recompute — flagged in-report as not yet promoted to a first-class part of the story | [board.md](reports/board.md) |
| **Full-table stat recompute** (`board_stats.tex`) | Keep the correlation table honest after the report rewrite | ✅ Tiny drift only (e.g. `material_imbalance` +0.128→+0.129, `player_clock_time` −0.139→−0.137) — consistent with a straight recompute, no methodology change | `reports/board_stats.tex` |

## 2026-07-06 {#2026-07-06}

> **BeFS tree-gen had no real backup: unvisited edges defaulted to a fake `0.0`, and selection compared frozen one-ply static values — poisoned the entire `befs1cp_md36` corpus + everything downstream (~148GB deleted). Rewrote BeFS/PUCT from scratch as a genuine best-first-minimax / corrected-FPU-PUCT class hierarchy, resolved a cp-vs-win-probability scale question, and relaunched the full pipeline overnight (BeFS base + PUCT variant, both on `value_feature: value`).**

| Description | Rationale | Status / finding | Reference |
|---|---|---|---|
| **Shipped default cost regime**: `mc_pack`/`train` `time_lambda` 18.537→10.0, `time_mode` `power_law`→`linear`, `fixed_budget` 48→96; new `eval:` config block added (`time_mode`/`time_lambda`/`maintenance_scale`/`maint_lambda`/`max_episodes`) | Move off the two-bucket `power_law` regime toward a single assessment regime for R-EVALUATE | ⚠️ Shipped this morning (commit `964e835`); superseded within the same 24h — the poisoning investigation below invalidated the corpus this was computed against, and the next day's entry found `time_lambda=10.0` itself ~1000–3000x miscalibrated once `value_feature` became win-probability-scaled. Kept here for provenance, not as a live setting | `config_minply15_maxply75.yaml` (commit `964e835`) |
| **Diagnosed BeFS tree-gen poisoning**: `EdgeStats()` defaults to `visit_count=0, total_value=0.0`; `_root_q_values_from_edge_stats`/`_select_leaf_by_befs` treated that fake `0.0` as a real value | Investigating why `evaluate.py`'s z_t-vs-tree-stats decodability collapsed after the mixed→single-budget switch | ❌ 63% of the 250K-tree corpus had a "winning" root move that was **never actually visited** (96.4% within `gss=0`); BeFS's greedy descent compares a node's *own frozen* one-ply static value forever, never anything backed up deeper — only 1 of 20-38 root children ever got expanded past ply 1. Deleted the poisoned corpus + all packed/materialized/encoder/controller data + `normative.md`/`budget_regime_investigation.md` (~148GB); `board.md` unaffected, `treesearch.md`/`pruning.md` predate the BeFS switch and are also unaffected | (superseded — no report yet) |
| **Confirmed against `ninarow`** (local van Opheusden BFS cognitive model, `bfs_node.h`) **and Korf & Chickering 1996** (Best-First Minimax / UBFM) | User asked how a real search model/literature handles FPU and the backup-vs-selection distinction | ✅ Both agree: a node's static eval seeds its value **once**, at creation; thereafter its value is *always* the backed-up minimax value of its children, recomputed to the root after every expansion; selection always follows the *current* best line, never a frozen snapshot | — |
| **Rewrote `teacher_targets.py`** as `TreeSearch`/`BeFSSearch`/`PUCTSearch`: BeFS seeds+overwrites a genuine max/min backup (no visit-averaging); PUCT's FPU fallback for an unvisited child changed from hardcoded `0.0` to that child's own (sign-flipped) static value, read-time only, never contaminating the real running mean | Make BeFS "a genuine BeFS" and give PUCT a principled FPU instead of the AlphaZero-style neutral-0 default | ✅ Verified independently (brute-force negamax by hand, not just by running the code) — caught and fixed 3 sign-derivation bugs in the adversarial test suite itself along the way. Full suite green (37 passed, 1 documented skip). Toy scripted case confirms BeFS *does* reconsider a sibling once a line's backed-up value degrades below it | `src/analysis/tests/test_befs_puct_rewrite.py` |
| **Real-FEN empirical check**: even with the fix, BeFS still tunnel-visions into ~1 of 20-38 root children past ply 1 on real positions | User's acceptance test ("depth should extend well beyond 1") | ⚠️ Confirmed *not* a bug — the leading line's backed-up value never actually drops below the runner-up's within a 96-node budget; that's correct greedy-best-first behavior (no exploration term, by definition). PUCT (which has an exploration term) spreads across far more root children on the *same* budget when both use win-probability, but **not** when both use `cp_order`: `c_puct=1.0` is calibrated for a bounded ~[-1,1] scale and is numerically inert against cp-magnitude Q values (confirmed: `c_puct=200` restores broadening) | — |
| **cp_order vs win-probability scale decision** | Shipped config used `value_feature: cp_order` for BeFS ("cp resolves the frontier ties pwin saturation creates"); found this also feeds `oracle_final_root_q_values`/`halt_reward` directly (not independently WDL-derived), making `mc_pack.min_halt_reward_range: 0.05` **vacuous** against cp-scale rewards | ✅ Resolved to single-currency **win-probability everywhere** (`value_feature: value` for both BeFS and PUCT): empirically, BeFS's ordinal child comparisons are ~identical under cp_order vs value (verified on 4 real positions, same touched-past-ply1 counts both ways) — the encoder/controller only ever see WDL-derived targets, so building the whole pipeline on that currency avoids the two-currency bug class outright and makes `min_halt_reward_range` meaningful again for free | `config_minply15_maxply75.yaml` |
| **SLURM CPU-utilization fix**: `sacct` flagged 125 jobs at ~49% util / 2 cores (job family `prune-regen`) | User received a cluster utilization report | ✅ Confirmed genuinely single-threaded (one `UciEngineProcess`, sequential Python loop, no pool) for `gen_trees.slurm` (successor of the deleted `prune_regen.slurm`) and `pack_root_merge.slurm` (sequential `merge_workers()`) — both `cpus-per-task` 2→1. Everything else checked out as genuinely parallel or left unconfirmed rather than guessed | — |
| **Config cleanup + overnight relaunch**: deleted `config_allply.yaml`/`config_budgetsweep_{b8,b16}.yaml`, repointed hardcoded defaults; added a `puct` variant (`from: gen_trees, to: eval`) alongside the existing `pruned` variant (whose `prune_epsilon` also got fixed 50cp→0.1 pwin, matching `UNITS["pwin"]["frac_good_eps"]` in `tree_loader.py`); submitted both chains (`RUN_VARIANTS=puct`) | Run BeFS (live path) and PUCT (comparison) overnight from a clean, verified config | ✅ Both chains confirmed starting cleanly (`filter_and_sample` ✅, both `gen_trees` arrays writing real tree files within the first few minutes, no errors). **Known cosmetic gap**: `gen_trees.slurm` derives its output dir from `human_analysis.trees_default` (a separate hardcoded field), not `treegen.output_dir` — so the `puct` variant's trees land in a directory still literally named `befs1cp_md36` (just isolated under `variants/puct/`, no collision/correctness issue). Manifest left in that directory explaining the mismatch | MANIFEST.md in `variants/puct/trees/befs1cp_md36/` |

## 2026-07-05 {#2026-07-05}

> **`board.md` trimmed to its core story and dual signed/absolute material-imbalance dashboards added; two config fixes landed alongside routine figure regens (`pruned` variant's override fields corrected to match `BuildTreeConfig`; constant per-episode `fixed_budget=48` replacing the old time-bucket sampling for the R-EVALUATE constant-cost assessment).**

| Description | Rationale | Status / finding | Reference |
|---|---|---|---|
| **`pruned` variant override fix**: `treegen.prune_eps` → `treegen.prune_mode: relative` + `treegen.prune_epsilon: 50.0` (cp) | The placeholder field name from `17705d4` (`prune_eps`) didn't match `BuildTreeConfig`'s real fields | ✅ | `config_minply15_maxply75.yaml` (commit `a94ec9b`) |
| **Constant per-episode budget**: new `mc_pack.fixed_budget: 48` / `train.fixed_budget: 48`; removed the old `scramble_min_time`/`medium_small_*`/`medium_large_*`/`large_*`/`very_large_*` time-bucket fields | The per-episode budget-bucket sampling conflated budget with episode length for the R-EVALUATE constant-cost assessment — a sampled bucket is not the same as a fixed per-episode budget | ✅ | `config_minply15_maxply75.yaml` (commit `b932972`) |
| **`board.md` simplified**: trimmed to the core story (legal-moves in-check mechanism, self-material jaggedness, material-imbalance signed+abs with staggered dips, checks-avail correctness/endgame/noise re-test, the material-interaction finding); dropped the stale smoke-vs-full provenance appendix and the checks piece-diversity/call-and-response digressions; deleted `outputs/reports/allply.md` outright | The report had accreted digressions and a provenance appendix that no longer earned its space | ✅ `material_imbalance` now tracked as **both** the signed quantity and a separate `abs_material_imbalance` dashboard ("dual material-imbalance curves"); every board/engine figure (incl. `board_stats.tex`) wiped and regenerated from scratch so nothing stale mixes with the current code | [board.md](reports/board.md) (commit `f64f618`) |

## 2026-07-03 {#2026-07-03}

> **PG readout froze (policy saturation) → stop-temperature anneal + minimal head; board features moved into preprocess; board.py unified. Same evening: preprocess/board pipeline collapsed back to 4 stages, board-scoped featurize pulled out of preprocess into a new `filter_and_sample` stage, a `variants:` mechanism added for nested pipeline forks (e.g. BeFS pruning), and `eval` wired up as a proper end-to-end pipeline stage.**
...
| **Pipeline collapse**: preprocess back to 4 wired stages (`get_games→shard→merge→process_moves`); captures/checks featurization folded into `process_moves` as an in-process spawn-`Pool` step (no orphan array / extra slurm files); `board.py` rewritten to 4 analyses (RT summary, per-covariate bivariate dashboards, correlation matrix, feature histograms) over the full ply-windowed table, no sampling | The earlier commit's captures/checks Slurm array was an orphan step outside the wired DAG; consolidate + dedup utils | ✅ 14 files changed (+247/−308); deleted `config.py`/`selected_db.py`/`board_features.py`, consolidated into `utils/helpers.py`; unified tree-values `cache_key` (was frozen at `100000_7`); `db_connection` routes DuckDB spill to scratch + caps memory; `lc0`→`engine` rename | `src/analysis/preprocess.py`, `src/analysis/board.py` (commit `af5dfd8`) |
| **New `filter_and_sample` stage**: board-scoped featurize (captures/checks) pulled OUT of the shared, run-independent preprocess and into a new run-scoped stage — `preprocess → processed_moves[_nonzero]` (pure SQL, run-independent) then `filter_and_sample → filtered_moves_<run>` (windowed + `game_fraction`, = CANON), which also samples the tree-root FEN set; `board --featurize/--merge/--plot` reads off the filtered table | Captures/checks featurization is a board precondition over the run's windowed subset, not something the 114M-row `personal.db` build should do for every FEN | ✅ New `src/analysis/filter_and_sample.py` (`build_filtered_moves`/`sample_tree_fens`, merges the old `filter_moves`+`filter_trees`); new `filter_and_sample.slurm`/`board_featurize.slurm`/`board_merge.slurm`; `engine.py` now reads `filtered_moves` directly (`create_ply_windowed_views` retired). Deploy note: the existing `personal.db` had stale captures/checks columns in `processed_moves`, needed a clean `process_moves` rerun first | `src/analysis/filter_and_sample.py`, `slurm/pipeline/filter_and_sample.slurm` (commit `459d6a3`) |
| **Nested normative variant branches**: config `variants:` block (`{from, to, share, overrides}`) lets a variant re-run a dependency-path slice of the pipeline with overrides while reusing the base run's outputs everywhere else (e.g. `pruned`: fork at `gen_trees` with `treegen.prune_epsilon` set, share `fens_sample`, through `eval`) | Need to A/B a pipeline-stage change (e.g. BeFS pruning, later PUCT-vs-BeFS) without duplicating the whole DAG or its outputs | ✅ `apply_variant()`/`_apply_variant()` in `render_stage.py`/`cts/_config.py`: forks `run_name`→`.../variants/<name>` (namespaces all outputs), pins `share`d keys to the base run's literal resolved paths, applies dotted overrides, drops the `variants` block before rendering. `submit_all.sh` gets `RUN_VARIANTS=<name>[,...]` — expands each into a parallel `@variant` job tail (intersection of descendants-of-`from` / ancestors-of-`to`). Test suite 73 passed / 2 skipped at the time | `slurm/pipeline/submit_all.sh`, `src/cts/_config.py::apply_variant` (commit `17705d4`) |
| **`eval` wired as a proper end-to-end pipeline stage**: `src/analysis/evaluate.py` rewritten (largest diff in the set); `baselines.py`/`ladder_plot.py`/`mchalt_scorer.py` deleted in favor of the `pg_controller_train.py`-fed eval path; new `outputs/reports/evaluate.md` | Commit has no message body — structural continuation of the same evening's pipeline restructure, making `eval` a first-class terminal DAG stage rather than a standalone script | ✅ 90 files changed (+623/−1365). Same commit also bundled housekeeping: added `fonts/HelveticaNeue-{Bold,Roman}.otf`, deleted the stale `outputs/figures/allply/**` and `smoke_*`-prefixed figures | `src/analysis/evaluate.py` (commit `67dce23`) |
...
| **Value-pruning plan + feasibility** | Scope the cost-profile experiment at scale | 📝 Plan staged: pick ε via completed-tree proxy on existing **250K n=1** trees → regen with pruning in PUCT → lock ε, fit normative model. **750K @ n=1 ≈ 1.5–3 h** (1.36 trees/s/proc), **~165 GB** (220 KB/tree; /scratch 95% full, free `elo1350` ~55 GB). Open decisions interviewed (rule=relative-to-best by leaf-eval value; regen 2–3 ε; 750K generated; oracle-then-meta-MDP). | [(R-PRUNING)](reports/pruning.md) |
| **Value-pruning PROXY (step 1) run** | Pick the ε grid (loose proxy: post-hoc prune the completed n=1 tree, negamax best=min-child) | ✅ `prune_proxy.py` on 250K trees (56s) → 262,286 human moves. **ε is a dial along size↔satisfaction**: ρ(pruned,RT) = −0.16 @ε=0 (satisfaction) → +0.32 @ε=2 (size = no-prune), crosses 0 @ε≈0.9. **No single ε beats the legal-moves floor (+0.34)**; partial\|legal non-zero both ends (−0.085 / +0.112). Loose proxy (final values, no budget redeploy) — confirms ε is the right knob, action at low ε. Chose regen grid **ε∈{0.05,0.2,0.5}**. | [(R-PRUNING)](reports/pruning.md) |
...
| **MC reports consolidation → `mc_minimal_plan.md`** | Four R-MC-* proposals (READOUT/COST/SIGNAL/DELIB) overlapped; §10b had already resolved the *objective* bug, leaving one open question (tree value ≈0 for stopping) | ✅ Folded all four into a single prioritized plan: **P0** Readout harness + regret-direct training (subsumes SIGNAL); **P1** cost-regime/λ sweep over *cached* traces (no gen) as the cheapest discriminator; **P2** Stockfish-provider trees + tiny GNN (1155× faster, but α-β → no child-WDL target); **P3** deliberation-value via PUCT-unstable + weak/Elo-limited teacher. Deleted the 4 reports, repointed index, resolved the dangling §10b ref. **Kept** `mc_pipeline.md` (still referenced by `pipeline.yaml`; holds the §10b diagnosis). | [(R-MC-PLAN)](../mc_minimal_plan.md) |
| **SF strength-ladder @ 50K — overnight tree-gen + full packing chain (P2)** | Build a Stockfish ladder (teacher-strength sweep) at parity with the lc0 MC pipeline; the earlier elo1800 stages-2–6 recipe was run interactively by a subagent and **never captured** (configs lost) | ✅ **Roots:** uniform-random **50K** sample of the ground-truth pool `fens.txt` → `sf_ladder_roots_50k.txt` (the prior 2K "smoke" file was the degenerate alphabetical *head* of fens.txt — lone-K+B endgames; the canonical `sampled_root_fens_2023.txt` is piece-placement-only → invalid for build_tree). Removed the old 150-tree smoke trees (1350/1800/full) + stale elo1800 downstream. **Rungs 1800/2000/2200** (replacing 1350/full per user). **Tree-gen:** 3 CPU arrays (20×2500), jobs 10323846/47/48 (~1.15 s/tree, 50K≈35 min). **Recipe reconstructed + validated end-to-end on 40 trees** (split→gnn-pack→encoder→mc-pack→materialize): split 80/20 seed 0; gnn-pack shard 2000; tiny encoder k=1 d_embed=32; mc-pack canonical oracle (λ=18.537/p=2.8/τ=2.5, maint off, buckets 1–120, 2/bucket, min_halt_reward_range=0.05) `num_workers=cpus`; materialize **needs num_workers≥1** (cost_sweep's `num_workers:0` div-by-zeros). **Committed** `slurm/1_preprocess_data/sf_pack_{cpu,gpu}.slurm` (recipe no longer lost) + `sf_ladder_{1800,2000,2200}.yaml`. **Chained:** per rung treegen→packCPU→packGPU via `afterok` (10324132–137). Outputs → `sf_{trees,packed,mc_packed,mc_materialized}/elo{1800,2000,2200}`. | [sf_pack_cpu.slurm](lmcos/slurm/1_preprocess_data/sf_pack_cpu.slurm) |

---

## 2026-06-18 {#2026-06-18}

Standardized the move-time analysis on the canonical trees; renamed metrics; consolidated reports.

| Description | Rationale | Status / finding | Reference |
|---|---|---|---|
| **Tree set — standardize on `human_trees`; delete local `lc0_trees`** | The local 150K set gave a weak/odd signal (r(stop,RT)≈+0.03) | ✅ Root cause = a **lexicographic-slice FEN population** (low-diversity), not a gen bug — metadata/net identical, same-FEN trees agree ~5e-3. Repointed to `human_trees` (real 2023 roots); r recovered to ~+0.12. Deleted `lc0_trees` (37 GB). Join by 4-field FEN, never filename. | [(R-DATA)](reports/reference.md) |
| **OSS → GSS (greedy stopping step)** | The cost-aware DP `optimal_stop_step` (λ=18.5) bails at median step 1 | ✅ Verified DP@cost0 == greedy (98.3%) and DP→greedy as cost→0. GSS = first expansion the best move is found; r(GSS,logRT)=+0.115. | [(R-MOVETIME-MODEL)](reports/movetime_model.md) |
| **VOC → "Gain"** (display rename; internal `voc` kept) | Clearer term | ✅ Renamed across code/figures/reports/decks; regenerated from cache (no recompute); values identical (+0.073). | [(R-MOVETIME-MODEL)](reports/movetime_model.md) |
| **Branching dominates → resource-rational proposal** | Branching ρ≈+0.35 ≫ any engine metric; realized Gain barely predicts RT | 📝 R-BRANCH: deliberation = metalevel computation to resolve *which move is best*; optimal stop grows with argmax-uncertainty ≈ policy entropy ≈ branching (a chess Hick's law). Gain-vs-GSS: branching sets scale (R²≈0.12), Gain adds unique *rank* variance only in non-decisive positions. | [(R-BRANCH)](reports/branching.md) |
| **Reports consolidation + archive merge** | Trim stale reports; one chronological log | ✅ Move-time split into board/model; human-dataset + tree-gen → [R-DATA](reports/reference.md); removed the LMCOS-model reports (GNN-pretrain, minimal-model) + their decks; merged the legacy lmcos archive into this notebook (§legacy) and removed the archive file. | [reports/reference.md](reports/reference.md) |
...
| **LMCOS — GNN child-WDL pretrain dataset decision** | Can the encoder be supervised-trained on ysagiv `human_trees`? | ✅ **No.** ysagiv `human_trees` (1.14M) have **no `edge_wdl_targets`** (verified over samples; only scalar `node_targets` + per-node WDL input features); `ChildWdlPretrainer` hard-requires edge targets and the node-target path was removed (`69d024e`). My `lc0_trees` (**150,003**, not 100K) **have** valid `edge_wdl_targets` (sum-to-1, no NaN). → train on `lc0_trees`. | [gnn-pretrain.md](#legacy) |
| **LMCOS — GNN pretrain SMOKE (256/64 trees, A100)** | Get a *functioning* training loop; confirm loss descends; <2 min/iter | ✅ Weighted child-WDL CE, Adam lr=1e-2, 15 ep, **840s (56s/ep)**. Train loss_gap **0.748→0.291**; val best 0.306 @ep10 then overfits (expected at lr=1e-2 on 256 trees). 26 `test_supervised_branch` pass. Machinery (per-epoch log, `*_resume.pt`, bucketed-KL JSONL, best enc/dec save) all already built. Job `9829331`, `qos=gpu-short` (Della rejects `--partition=gpu`). | [plot](lmcos/analysis/figures/pretrain_smoke_loss.png) · [report](#legacy) |
| **LMCOS — full-run ETA caveat** | Size the 150K run | ⏳ 56s/ep is **CPU-tensorization-bound** (raw loader, `num_workers=0`) → naive 9h/ep — do NOT run raw. **Pre-pack once** (`preprocess_gnn.pack` → tensorized shards, as ysagiv does) then measure one packed epoch. #epochs-to-converge must be read off the real-run val plateau, not this overfit smoke. | [gnn-pretrain.md](#legacy) |
| **Human analytics — audit (skeptical subagent)** | Filtering correctness of kept figures | ⚠️ **MQ is Stockfish d5, not LC0** (`build_pos_with_engine_eval.py:261` default). The "FULL" MQ plot is a **1M-row ply-15–75, opp_clock≥60 subset** (not 135M). Ply-tertile labels (cuts 27/56) misapplied to that subset → "Early" panel has no openings. Instant-move exclusion biases Ply×MT (U-shaped zero-fraction). Tree OSS/VOC subset spans full ply → not population-matched to MQ. FEN-only join fan-out fragile. Units/log/OSS-def verified fine. | — |
| **Human analytics — MQ↔MT mystery (subagent)** | Why negative MQ↔MT? | ℹ️ **Not a sign bug — a difficulty confound.** Spearman ρ=−0.119 (N=1M), survives all controls/de-meaning; long thinks land on hard/high-branching/high-VOC positions (best-move rate 0.62→0.49 across MT deciles). MQ×MT×branching interaction β=−0.007 (t=−7): branching amplifies but small. Rec: recompute MQ from **LC0** + report difficulty-residualized partials. | — |
...
| **R-BATCHGEN plan** — batched in-process tree-gen, parity-gated; layered parity contract; JAX analysis | Route-b from R-U1-SPEED: 10–100× + native tree exposure | ✅ Report written; the only way to both speed up *and* own the tree/snapshots | [(R-BATCHGEN)](reports/reference.md) |
| **L1 — batched search loop** (`generate_trees_batched`, frontier-batched PUCT reusing legacy helpers) | Speed via batching across independent trees; per-tree order preserved | ✅ **Byte-exact to `build_pretrain_example`** under a fixed evaluator (T-replay). The load-bearing gate. | [(R-BATCHGEN §3)](reports/reference.md) |
| **Phase 2 — in-process net** (`NetEvaluator`, lczerolens + `leela2onnx` ONNX) | Replace batch-1 lc0-UCI oracle | ✅ Committed `3a2bf7d`. `re_baseline=False` reproduces lc0 **exactly** on midgame FENs | [(R-BATCHGEN §10)](reports/reference.md) |
| **3 lc0 quirks pinned** — ~~value=1-ply best-child valuehead minimax~~ (⚠️ **DISPROVEN — value is the RAW value-head**, see CORRECTIONS); priors at PolicyTemperature 1.359; history `fen_only`=lczerolens REPEATED | Bit-for-bit replication (user: replicate first, flip `re_baseline=True` later) | ✅ priors/history confirmed vs binary (priors d2d4 0.1806 vs 0.1804); ❌ the "1-ply" value pin was a startpos-only artifact (`value==win−loss` over 55k nodes ⇒ raw head) | [(R-BATCHGEN §2)](reports/reference.md) |
| **3 real bugs caught by the pinning test** — `encode_move(move, us)`; onnx2torch needs `.eval()` (BatchNorm batch-dependence); position specs (`\|\|moves\|\|`) need `split_position_spec` | Each would have silently corrupted the 150K dataset | ✅ All fixed; pinning test green | [(R-BATCHGEN §10)](reports/reference.md) |
| **Speedup go/no-go** — A100 bench (budget 16, extrapolated to 96) | The §8 ≥5× gate before scaling | ❌ **NO-GO.** raw `re_baseline=True` ~8 s/tree (**~2×**); faithful 1-ply ~125 s/tree (**~7× slower**). Bottleneck = Python 112-plane encoding (`to_input_tensor`), not the GPU. Below the 5× gate → not worth the build. | [(R-BATCHGEN)](reports/reference.md) |
...
| Description | Rationale | Status / finding | Reference |
|---|---|---|---|
| **U1.0 smoke** — Lc0-GPU / Lc0-CPU / Stockfish on 100 warm human FENs (budget 96) | Pin per-tree cost; decide engine before 50K | ✅ Lc0-GPU **16.87 s/tree** (A100, steady); Lc0-CPU **~738 s/tree** (4-core, pure-CPU node); SF 0.45–11.65 s/pos (d12–20), 0.026 s @100k nodes | [(R-U1)](reports/reference.md) |
| **CPU-lane gate** — lc0-blas on a pure-CPU node | The 50K plan is CPU-led | ⚠️→✅ **Blocked then FIXED:** CUDA-linked lc0 needs `libcublas.so.12`; absent on CPU nodes → crash. Fix: `LD_LIBRARY_PATH` → venv `nvidia/*/lib` (no rebuild). Lane viable. | [(R-U1)](reports/reference.md) |
...
| **U1.1 fire** — faithful 50K, GPU-only | Generate the reunified trees | 🚀 Job `9282935`, 50×1000-FEN shards, `gpu-short %20`, `resume:true`; coexists with an unrelated `prod_full` job (untouched). ~3 d at low concurrency, faster if GPUs free. | [(R-U1)](reports/reference.md) |
...
| **U3 model comparison** — controller vs baselines on existing diagnostics (no GPU/new trees) | Quantify controller lift; parallel thread | ✅ On 30,630 episodes (`subtree_weighting_root_budget`): **controller avg-regret +0.026**; exact-stop **63%**. | [(R-U3)](#legacy) |
| **U3 semi-smart baselines** — value-plateau + fixed-fraction-of-budget (+4 tests, 16 total) | Stronger non-trivial anchors | ✅ **Fixed-fraction (0.25 budget) is best baseline** (+0.069, beats gain-depth +0.148 & value-based rules) → stop is budget-structure-driven. **Value-plateau under-performs gain-depth** (+0.241; smoothing delays the early stops the oracle wants). Controller still wins (~2.6×). | [(R-U3)](#legacy) |
| **U3 cost-vs-value decomposition + slides** — why fixed-fraction competes | hl4291 hypothesis: budget over-weighted; regret weak | ✅ Oracle stops **bimodal**: ~49% value-driven (cost≈0), ~28% **cost-forced** (budget depleted). Stops **early** (median 3.8% of budget; corr 0.41). Regret weak among good rules → **exact-stop** discriminates (0.63 vs 0.19 vs 0.10). Levers: `time_lambda`, budget dist. 2 figures added to deck. | [(R-U3)](#legacy) |
...
| Description | Rationale | Status / finding | Reference |
|---|---|---|---|
| **Docs restructure** — root `labnotebook.md` + `reports/`; drop `proposed_next_steps.md` | Single chronology + procedure checklists for open work | ✅ Open steps live in active `R-*` reports | [reports/reference.md](reports/reference.md) |
...
| **LMCOS A2** — tree-stats + tiny GNN scratch (Config D) | Skip 1-day pretrain if small encoder suffices | ⏳ Config D YAML open; child-WDL pretrain job **9215254** started | [(R-A2)](#legacy) |
...
| Description | Rationale | Status / finding | Reference |
|---|---|---|---|
| **LMCOS layout** — `cts.*`, `slurm/<stage>/` | Stage-owned paths | ✅ `submit_configs.sh` | [(R-ARCH-LMCOS)](#legacy) |
...
| Description | Rationale | Status / finding | Reference |
|---|---|---|---|
| **Human dataset** — Lichess 10+0, DuckDB | RT / engine foundation | ✅ 135M nonzero-RT moves | [(R-DATA)](reports/reference.md) |
...
| **LMCOS history** — Apr–May 2026 | Encoder / topology arc | ✅ Archived | [(R-ARCH-LMCOS)](#legacy) |

---

## 📦 Legacy lmcos lab notebook (Apr–May 2026) {#legacy}

> Merged verbatim from the former `reports/archive-lmcos-notebook-legacy.md` (the original lmcos lab notebook) on 2026-06-18; headers demoted one level.
