...
# Logs: human_analytics/slurm/logs/eval_*.out (see engine_eval_shard.sbatch)
#
# Usage (same PROJECT_DIR convention as preprocess.sh):
#   bash /home/hl4291/chess_analysis/human_analytics/slurm/engine_eval.sh
#   ENGINE=lc0 bash human_analytics/slurm/engine_eval.sh
#   bash human_analytics/slurm/engine_eval.sh --merge-only
#   bash human_analytics/slurm/engine_eval.sh --no-wait   # submit only; then --merge-only when the array is done
...
run_merge_and_join() {
  ...
  python3 human_analytics/slurm/scripts/script_engine_eval.py merge --engine "${ENGINE}" --db "${DB}" --input_dir "${INPUT_DIR}"
...
if [[ "${NO_WAIT}" -eq 1 ]]; then
  ...
  JID=$(
    sbatch --parsable --export=ALL,ENGINE="${ENGINE}",LIMIT="${LIMIT}" \
      "${PROJECT_DIR}/human_analytics/slurm/engine_eval_shard.sbatch"
  )
  echo "Submitted Slurm array job: ${JID} (logs: human_analytics/slurm/logs/eval_*.out)"
  echo "Not waiting (--no-wait). When the job completes, run:"
  echo "  bash human_analytics/slurm/engine_eval.sh --merge-only --engine ${ENGINE} --db ${DB} --input-dir ${INPUT_DIR}"
  exit 0
fi
...
set -e
if [[ "${_wc}" -ne 0 ]]; then
  echo "sbatch --wait failed (exit ${_wc}); not running merge. Check human_analytics/slurm/logs/ and re-run with --merge-only when parquets are ready." >&2
  exit "${_wc}"
fi
echo "Array job ${JID} finished at $(date). Logs: human_analytics/slurm/logs/eval_*.out"
