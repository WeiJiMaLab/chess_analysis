# Migration Plan: lmcos to human_analytics/metacontrol
...
3. [x] Create `human_analytics/metacontrol/core/` and move the baseline tree logic.
4. [x] Harden engine providers (LC0, Stockfish), fix perspective bugs, and establish engine-invariant tests.
5. [x] Implement `human_analytics/metacontrol/scripts/sample.py` for root FEN extraction from Lichess DB (`ChessSampler`), plus fast `compose_full_fen` unit tests (`pytest -m "not integration"`).
