# lmcos_small — self-contained Stockfish mini-GNN / mini-MC halt-model pipeline
...
```bash
source env.sh        # PYTHONPATH=src (this fork) + venv
```
...
figures/normative/              phase 4 — regret/oss/ladder plots + results JSON
```
...
in `../lmcos` (`mc_pipeline.md`, `labnotebook.md`). If the parent's `cts`
